...
public:
    ...
    SVGLengthValue blend(const SVGLengthValue& from, float progress) const
    {
        ...
        if (fromType == LengthTypePercentage || toType == LengthTypePercentage) {
            ...
            auto result = length.newValueSpecifiedUnits(LengthTypePercentage, CyberCore::blend(fromPercent, toPercent, progress));
        ...
        if (fromType == toType || from.isZero() || isZero() || fromType == LengthTypeEMS || fromType == LengthTypeEXS) {
            ...
            if (isZero()) {
                auto result = length.newValueSpecifiedUnits(fromType, CyberCore::blend(fromValue, toValue, progress));
                if (result.hasException())
                    return { };
            } else {
                auto result = length.newValueSpecifiedUnits(toType, CyberCore::blend(fromValue, toValue, progress));
                if (result.hasException())
                    return { };
            }
            return length;
        ...
        auto result = length.newValueSpecifiedUnits(toType, CyberCore::blend(fromValue.releaseReturnValue(), toValue, progress));
        if (result.hasException())
            return { };
        return length;
    }
