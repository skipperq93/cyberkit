...
#include "config.h"
#include "CyberKitSoupRequestGeneric.h"
...
G_DEFINE_TYPE(CyberKitSoupRequestGeneric, webkit_soup_request_generic, SOUP_TYPE_REQUEST)

struct _CyberKitSoupRequestGenericPrivate {
    CString mimeType;
...
{
    WEBKIT_SOUP_REQUEST_GENERIC(object)->priv->~CyberKitSoupRequestGenericPrivate();
    G_OBJECT_CLASS(webkit_soup_request_generic_parent_class)->finalize(object);
}

static void webkit_soup_request_generic_init(CyberKitSoupRequestGeneric* request)
{
    CyberKitSoupRequestGenericPrivate* priv = G_TYPE_INSTANCE_GET_PRIVATE(request, WEBKIT_TYPE_SOUP_REQUEST_GENERIC, CyberKitSoupRequestGenericPrivate);
    request->priv = priv;
    new (priv) CyberKitSoupRequestGenericPrivate();
}
...
{
    CyberKitSoupRequestGenericClient* client = WEBKIT_SOUP_REQUEST_GENERIC_GET_CLASS(request)->client;
    ASSERT(client);
...
static void webkit_soup_request_generic_class_init(CyberKitSoupRequestGenericClass* requestGenericClass)
{
    ...
    g_type_class_add_private(requestGenericClass, sizeof(CyberKitSoupRequestGenericPrivate));
}

void webkitSoupRequestGenericSetContentLength(CyberKitSoupRequestGeneric* request, goffset contentLength)
{
    request->priv->contentLength = contentLength;
}

void webkitSoupRequestGenericSetContentType(CyberKitSoupRequestGeneric* request, const char* mimeType)
{
    request->priv->mimeType = mimeType;
}

void webkitSoupRequestGenericSetRequest(CyberKitSoupRequestGeneric* request, const ResourceRequest& resourceRequest)
{
    request->priv->resourceRequest = resourceRequest;
}

const ResourceRequest& webkitSoupRequestGenericGetRequest(CyberKitSoupRequestGeneric* request)
{
