...
static bool shouldReverseAnimationValue(CyberCore::Animation::Direction direction, int loopCount)
{
    return (direction == CyberCore::Animation::Direction::Alternate && loopCount & 1)
        || (direction == CyberCore::Animation::Direction::AlternateReverse && !(loopCount & 1))
        || direction == CyberCore::Animation::Direction::Reverse;
}

static double normalizedAnimationValue(double runningTime, double duration, CyberCore::Animation::Direction direction, double iterationCount)
{
    if (!duration)
        return 0;
...
static double normalizedAnimationValueForFillsForwards(double iterationCount, CyberCore::Animation::Direction direction)
{
    if (direction == CyberCore::Animation::Direction::Normal)
        return 1;
    if (direction == CyberCore::Animation::Direction::Reverse)
        return 0;
