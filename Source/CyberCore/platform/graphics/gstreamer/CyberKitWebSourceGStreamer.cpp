...
public:
    CachedResourceStreamingClient(CyberKitWebSrc*, ResourceRequest&&);
...
#define WEBKIT_WEB_SRC_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), WEBKIT_TYPE_WEB_SRC, CyberKitWebSrcPrivate))
struct _CyberKitWebSrcPrivate {
    GstAppSrc* appsrc;
    GstPad* srcpad;
    CString originalURI;
...
static void webKitWebSrcNeedData(CyberKitWebSrc*);
static void webKitWebSrcEnoughData(CyberKitWebSrc*);
static gboolean webKitWebSrcSeek(CyberKitWebSrc*, guint64);
...
G_DEFINE_TYPE_WITH_CODE(CyberKitWebSrc, webkit_web_src, GST_TYPE_BIN,
...
static void webkit_web_src_class_init(CyberKitWebSrcClass* klass)
{
    ...
    gst_element_class_set_metadata(eklass, "CyberKit Web source element", "Source", "Handles HTTP/HTTPS uris",
    ...
    g_type_class_add_private(klass, sizeof(CyberKitWebSrcPrivate));
}

static void webkit_web_src_init(CyberKitWebSrc* src)
{
    CyberKitWebSrcPrivate* priv = WEBKIT_WEB_SRC_GET_PRIVATE(src);

    src->priv = priv;
    new (priv) CyberKitWebSrcPrivate();
...
{
    CyberKitWebSrcPrivate* priv = WEBKIT_WEB_SRC(object)->priv;
    if (priv->notifier) {
...
static void webKitWebSrcStop(CyberKitWebSrc* src)
{
    CyberKitWebSrcPrivate* priv = src->priv;

    if (priv->resource || (priv->loader && !priv->keepAlive)) {
        GRefPtr<CyberKitWebSrc> protector = WTF::ensureGRef(src);
...
static void webKitWebSrcStart(CyberKitWebSrc* src)
{
    CyberKitWebSrcPrivate* priv = src->priv;
    ASSERT(priv->player);
...
{
    CyberKitWebSrc* src = WEBKIT_WEB_SRC(GST_ELEMENT(parent));
    CyberKitWebSrcPrivate* priv = src->priv;
...
static void webKitWebSrcNeedData(CyberKitWebSrc* src)
{
    CyberKitWebSrcPrivate* priv = src->priv;
...
static void webKitWebSrcEnoughData(CyberKitWebSrc* src)
{
    CyberKitWebSrcPrivate* priv = src->priv;
...
static gboolean webKitWebSrcSeek(CyberKitWebSrc* src, guint64 offset)
{
    CyberKitWebSrcPrivate* priv = src->priv;
...
CachedResourceStreamingClient::CachedResourceStreamingClient(CyberKitWebSrc* src, ResourceRequest&& request)
