...
{
#if USE(GSTREAMER_GL)
    if (m_videoDecoderPlatform == CyberKitGstVideoDecoderPlatform::Video4Linux)
        flushCurrentBuffer();
#endif
...
void MediaPlayerPrivateGStreamerBase::flushCurrentBuffer()
{
    ...
    bool shouldWait = m_videoDecoderPlatform == CyberKitGstVideoDecoderPlatform::Video4Linux;
...
void MediaPlayerPrivateGStreamerBase::updateTextureMapperFlags()
{
    ...
    // convert it.
    if (m_videoDecoderPlatform != CyberKitGstVideoDecoderPlatform::ImxVPU)
...
GstElement* MediaPlayerPrivateGStreamerBase::createVideoSinkGL()
{
    ...
    if (!appsink || !upload || !colorconvert) {
        ...
        g_warning("CyberKit wasn't able to find the GStreamer opengl plugin. Hardware-accelerated zero-copy video rendering can't be enabled without this plugin.");
        return nullptr;
    }
