...
static bool handleKeyResponse(CyberKitMediaCommonEncryptionDecrypt* self, RefPtr<CDMInstance>);
static bool decrypt(CyberKitMediaCommonEncryptionDecrypt*, GstBuffer* iv, GstBuffer* keyid, GstBuffer* sample, unsigned subSamplesCount, GstBuffer* subSamples);
...
static bool handleKeyResponse(CyberKitMediaCommonEncryptionDecrypt* self, RefPtr<CDMInstance> cdmInstance)
{
...
static bool findAndSetKey(CyberKitMediaClearKeyDecryptPrivate* priv, const Ref<SharedBuffer>& keyID)
{
...
static bool decrypt(CyberKitMediaCommonEncryptionDecrypt* self, GstBuffer* ivBuffer, GstBuffer* keyIDBuffer, GstBuffer* buffer, unsigned subSampleCount, GstBuffer* subSamplesBuffer)
{
    if (!ivBuffer) {
    ...
    auto mappedIVBuffer = CyberCore::GstMappedBuffer::create(ivBuffer, GST_MAP_READ);
    if (!mappedIVBuffer) {
    ...
    auto mappedKeyIdBuffer = CyberCore::GstMappedBuffer::create(keyIDBuffer, GST_MAP_READ);
    if (!mappedKeyIdBuffer) {
        GST_ERROR_OBJECT(self, "Failed to map key id buffer");
        return false;
    }

    auto mappedBuffer = CyberCore::GstMappedBuffer::create(buffer, GST_MAP_READWRITE);
    if (!mappedBuffer) {
    ...
    auto mappedSubSamplesBuffer = CyberCore::GstMappedBuffer::create(subSamplesBuffer, GST_MAP_READ);
