...
#include "CyberKitMediaSourceGStreamerPrivate.h"
...
static Stream* getStreamByAppsrc(CyberKitMediaSrc*, GstElement*);
static void seekNeedsDataMainThread(CyberKitMediaSrc*);
static void notifyReadyForMoreSamplesMainThread(CyberKitMediaSrc*, Stream*);

static void enabledAppsrcNeedData(GstAppSrc* appsrc, guint, gpointer userData)
{
    CyberKitMediaSrc* webKitMediaSrc = static_cast<CyberKitMediaSrc*>(userData);
    ...
    if (allAppsrcNeedDataAfterSeek) {
        ...
        case MediaSourceSeekToTime:
            webKitMediaSrc->priv->notifier->notify(CyberKitMediaSrcMainThreadNotification::SeekNeedsData, [webKitMediaSrc] {
        ...
        if (appsrcStream && appsrcStream->type != CyberCore::Invalid)
            webKitMediaSrc->priv->notifier->notify(CyberKitMediaSrcMainThreadNotification::ReadyForMoreSamples, [webKitMediaSrc, appsrcStream] {
...
static void enabledAppsrcEnoughData(GstAppSrc *appsrc, gpointer userData)
{
    ...
    if (!stream || stream->type == CyberCore::Invalid)
        return;
...
static Stream* getStreamByAppsrc(CyberKitMediaSrc* source, GstElement* appsrc)
{
...
G_DEFINE_TYPE_WITH_CODE(CyberKitMediaSrc, webkit_media_src, GST_TYPE_BIN,
...
static void webkit_media_src_class_init(CyberKitMediaSrcClass* klass)
{
    ...
    gst_element_class_set_static_metadata(eklass, "CyberKit Media source element", "Source", "Handles Blob uris", "Stephane Jadaud <sjadaud@sii.fr>, Sebastian Dröge <sebastian@centricular.com>, Enrique Ocaña González <eocanha@igalia.com>");
    ...
    webKitMediaSrcSignals[SIGNAL_VIDEO_CHANGED] =
        ...
        G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(CyberKitMediaSrcClass, videoChanged), nullptr, nullptr,
        g_cclosure_marshal_generic, G_TYPE_NONE, 0, G_TYPE_NONE);
    webKitMediaSrcSignals[SIGNAL_AUDIO_CHANGED] =
        g_signal_new("audio-changed", G_TYPE_FROM_CLASS(oklass),
        G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(CyberKitMediaSrcClass, audioChanged), nullptr, nullptr,
        g_cclosure_marshal_generic, G_TYPE_NONE, 0, G_TYPE_NONE);
    webKitMediaSrcSignals[SIGNAL_TEXT_CHANGED] =
        g_signal_new("text-changed", G_TYPE_FROM_CLASS(oklass),
        G_SIGNAL_RUN_LAST,
        G_STRUCT_OFFSET(CyberKitMediaSrcClass, textChanged), nullptr, nullptr,
        g_cclosure_marshal_generic, G_TYPE_NONE, 0, G_TYPE_NONE);

    eklass->change_state = webKitMediaSrcChangeState;

    g_type_class_add_private(klass, sizeof(CyberKitMediaSrcPrivate));
}

static GstFlowReturn webkitMediaSrcChain(GstPad* pad, GstObject* parent, GstBuffer* buffer)
{
    GRefPtr<CyberKitMediaSrc> self = adoptGRef(WEBKIT_MEDIA_SRC(gst_object_get_parent(parent)));
...
static void webkit_media_src_init(CyberKitMediaSrc* source)
{
    source->priv = WEBKIT_MEDIA_SRC_GET_PRIVATE(source);
    new (source->priv) CyberKitMediaSrcPrivate();
    ...
    source->priv->notifier = CyberCore::MainThreadNotifier<CyberKitMediaSrcMainThreadNotification>::create();
...
void webKitMediaSrcFinalize(GObject* object)
{
    ...
    CyberKitMediaSrcPrivate* priv = source->priv;
...
void webKitMediaSrcDoAsyncStart(CyberKitMediaSrc* source)
{
...
void webKitMediaSrcDoAsyncDone(CyberKitMediaSrc* source)
{
    CyberKitMediaSrcPrivate* priv = source->priv;
    if (priv->asyncStart) {
        GST_BIN_CLASS(parent_class)->handle_message(GST_BIN(source),
...
gint64 webKitMediaSrcGetSize(CyberKitMediaSrc* webKitMediaSrc)
{
    gint64 duration = 0;
    for (Stream* stream : webKitMediaSrc->priv->streams)
...
{
    CyberKitMediaSrc* source = WEBKIT_MEDIA_SRC(GST_ELEMENT(parent));
        ...
        case GST_FORMAT_TIME: {
            if (source->priv && source->priv->mediaPlayerPrivate) {
                ...
                if (duration > MediaTime::zeroTime()) {
                    gst_query_set_duration(query, format, CyberCore::toGstClockTime(duration));
                    GST_DEBUG_OBJECT(source, "Answering: duration=%" GST_TIME_FORMAT, GST_TIME_ARGS(CyberCore::toGstClockTime(duration)));
                    result = TRUE;
                }
            }
            break;
...
void webKitMediaSrcUpdatePresentationSize(GstCaps* caps, Stream* stream)
{
    ...
    if (CyberCore::doCapsHaveType(caps, GST_VIDEO_CAPS_TYPE_PREFIX)) {
        Optional<CyberCore::FloatSize> size = CyberCore::getVideoResolutionFromCaps(caps);
        if (size.hasValue())
            stream->presentationSize = size.value();
        else
            stream->presentationSize = CyberCore::FloatSize();
    } else
        stream->presentationSize = CyberCore::FloatSize();
...
void webKitMediaSrcLinkStreamToSrcPad(GstPad* sourcePad, Stream* stream)
{
    ...
    GstPad* ghostpad = CyberCore::webkitGstGhostPadFromStaticTemplate(&srcTemplate, padName.get(), sourcePad);
...
void webKitMediaSrcFreeStream(CyberKitMediaSrc* source, Stream* stream)
{
    ...
    GST_OBJECT_LOCK(source);
    switch (stream->type) {
    case CyberCore::Audio:
        source->priv->numberOfAudioStreams--;
        break;
    case CyberCore::Video:
    ...
    if (stream->type != CyberCore::Invalid) {
...
void webKitMediaSrcCheckAllTracksConfigured(CyberKitMediaSrc* webKitMediaSrc)
{
...
static void seekNeedsDataMainThread(CyberKitMediaSrc* source)
{
    ...
    CyberCore::MediaPlayerPrivateGStreamerMSE* mediaPlayerPrivate = source->priv->mediaPlayerPrivate;
...
static void notifyReadyForMoreSamplesMainThread(CyberKitMediaSrc* source, Stream* appsrcStream)
{
    GST_OBJECT_LOCK(source);
...
void webKitMediaSrcSetMediaPlayerPrivate(CyberKitMediaSrc* source, CyberCore::MediaPlayerPrivateGStreamerMSE* mediaPlayerPrivate)
{
    GST_OBJECT_LOCK(source);
...
void webKitMediaSrcSetReadyForSamples(CyberKitMediaSrc* source, bool isReady)
{
    if (source) {
        GST_OBJECT_LOCK(source);
        for (Stream* stream : source->priv->streams)
...
void webKitMediaSrcPrepareSeek(CyberKitMediaSrc* source, const MediaTime& time)
{
    GST_OBJECT_LOCK(source);
...
void webKitMediaSrcPrepareInitialSeek(CyberKitMediaSrc* source, double rate, const MediaTime& startTime, const MediaTime& endTime)
{
    GST_OBJECT_LOCK(source);
    ...
    gst_segment_do_seek(segment.get(), rate, GST_FORMAT_TIME,
        ...
        GST_SEEK_TYPE_SET, CyberCore::toGstUnsigned64Time(startTime),
        GST_SEEK_TYPE_SET, CyberCore::toGstUnsigned64Time(endTime), nullptr);
