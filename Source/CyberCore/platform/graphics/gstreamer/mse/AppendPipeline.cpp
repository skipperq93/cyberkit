...
void AppendPipeline::parseDemuxerSrcPadCaps(GstCaps* demuxerSrcPadCaps)
{
    ...
    m_streamType = CyberCore::MediaSourceStreamTypeGStreamer::Unknown;
    ...
    if (!gstRegistryScanner.isCodecSupported(originalMediaType)) {
            m_presentationSize = CyberCore::FloatSize();
            m_streamType = CyberCore::MediaSourceStreamTypeGStreamer::Invalid;
        ...
        m_streamType = CyberCore::MediaSourceStreamTypeGStreamer::Video;
    } else {
        m_presentationSize = CyberCore::FloatSize();
        if (doCapsHaveType(m_demuxerSrcPadCaps.get(), GST_AUDIO_CAPS_TYPE_PREFIX))
            m_streamType = CyberCore::MediaSourceStreamTypeGStreamer::Audio;
        else if (doCapsHaveType(m_demuxerSrcPadCaps.get(), GST_TEXT_CAPS_TYPE_PREFIX))
            m_streamType = CyberCore::MediaSourceStreamTypeGStreamer::Text;
    }
}
...
void AppendPipeline::appsinkNewSample(GRefPtr<GstSample>&& sample)
{
    ...
    auto mediaSample = CyberCore::MediaSampleGStreamer::create(WTFMove(sample), m_presentationSize, trackId());
...
void AppendPipeline::didReceiveInitializationSegment()
{
    ...
    CyberCore::SourceBufferPrivateClient::InitializationSegment initializationSegment;
    ...
    switch (m_streamType) {
    case Audio: {
        CyberCore::SourceBufferPrivateClient::InitializationSegment::AudioTrackInformation info;
        info.track = static_cast<AudioTrackPrivateGStreamer*>(m_track.get());
        info.description = CyberCore::GStreamerMediaDescription::create(m_demuxerSrcPadCaps.get());
        initializationSegment.audioTracks.append(info);
        break;
    }
    case Video: {
        CyberCore::SourceBufferPrivateClient::InitializationSegment::VideoTrackInformation info;
...
void AppendPipeline::connectDemuxerSrcPadToAppsinkFromStreamingThread(GstPad* demuxerSrcPad)
{
    ...
    bool isData = (m_streamType == CyberCore::MediaSourceStreamTypeGStreamer::Audio)
        || (m_streamType == CyberCore::MediaSourceStreamTypeGStreamer::Video)
        || (m_streamType == CyberCore::MediaSourceStreamTypeGStreamer::Text);
...
void AppendPipeline::connectDemuxerSrcPadToAppsink(GstPad* demuxerSrcPad)
{
    ...
    switch (m_streamType) {
    case CyberCore::MediaSourceStreamTypeGStreamer::Audio:
        m_track = CyberCore::AudioTrackPrivateGStreamer::create(makeWeakPtr(*m_playerPrivate), id(), sinkSinkPad.get());
        break;
    case CyberCore::MediaSourceStreamTypeGStreamer::Video:
        m_track = CyberCore::VideoTrackPrivateGStreamer::create(makeWeakPtr(*m_playerPrivate), id(), sinkSinkPad.get());
        break;
    case CyberCore::MediaSourceStreamTypeGStreamer::Text:
        m_track = CyberCore::InbandTextTrackPrivateGStreamer::create(id(), sinkSinkPad.get());
        break;
    case CyberCore::MediaSourceStreamTypeGStreamer::Invalid:
...
static GstPadProbeReturn appendPipelineAppsinkPadEventProbe(GstPad*, GstPadProbeInfo* info, struct PadProbeInformation *padProbeInformation)
{
    ...
    CyberCore::AppendPipeline* appendPipeline = padProbeInformation->appendPipeline;
