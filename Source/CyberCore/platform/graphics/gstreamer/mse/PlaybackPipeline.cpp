...
static Stream* getStreamByTrackId(CyberKitMediaSrc*, AtomicString);
static Stream* getStreamBySourceBufferPrivate(CyberKitMediaSrc*, CyberCore::SourceBufferPrivateGStreamer*);

static Stream* getStreamByTrackId(CyberKitMediaSrc* source, AtomicString trackIdString)
{
    // CyberKitMediaSrc should be locked at this point.
...
static Stream* getStreamBySourceBufferPrivate(CyberKitMediaSrc* source, CyberCore::SourceBufferPrivateGStreamer* sourceBufferPrivate)
{
    for (Stream* stream : source->priv->streams) {
...
void PlaybackPipeline::setCyberKitMediaSrc(CyberKitMediaSrc* webKitMediaSrc)
{
    GST_DEBUG("webKitMediaSrc=%p", webKitMediaSrc);
    m_webKitMediaSrc = webKitMediaSrc;
}

CyberKitMediaSrc* PlaybackPipeline::webKitMediaSrc()
{
...
{
    CyberKitMediaSrcPrivate* priv = m_webKitMediaSrc->priv;
...
{
    CyberKitMediaSrc* webKitMediaSrc = m_webKitMediaSrc.get();
    ...
    if (doCapsHaveType(caps, GST_AUDIO_CAPS_TYPE_PREFIX)) {
        ...
        stream->audioTrack = RefPtr<CyberCore::AudioTrackPrivateGStreamer>(static_cast<CyberCore::AudioTrackPrivateGStreamer*>(trackPrivate.get()));
    } else if (doCapsHaveType(caps, GST_VIDEO_CAPS_TYPE_PREFIX)) {
        stream->type = Video;
        stream->parent->priv->numberOfVideoStreams++;
        signal = SIGNAL_VIDEO_CHANGED;
        stream->videoTrack = RefPtr<CyberCore::VideoTrackPrivateGStreamer>(static_cast<CyberCore::VideoTrackPrivateGStreamer*>(trackPrivate.get()));
...
void PlaybackPipeline::notifyDurationChanged()
{
    ...
    // CyberKitMediaSrc will ask MediaPlayerPrivateGStreamerMSE for the new duration later, when somebody asks for it.
}
...
void PlaybackPipeline::enqueueSample(Ref<MediaSample>&& mediaSample)
{
    ...
    GST_TRACE("enqueing sample trackId=%s PTS=%f presentationSize=%.0fx%.0f at %" GST_TIME_FORMAT " duration: %" GST_TIME_FORMAT,
        ...
        GST_TIME_ARGS(CyberCore::toGstClockTime(mediaSample->presentationTime())),
        GST_TIME_ARGS(CyberCore::toGstClockTime(mediaSample->duration())));
