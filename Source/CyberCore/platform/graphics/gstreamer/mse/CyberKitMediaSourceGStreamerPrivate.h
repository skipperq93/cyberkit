...
#define WEBKIT_MEDIA_SRC_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), WEBKIT_TYPE_MEDIA_SRC, CyberKitMediaSrcPrivate))

typedef struct _Stream Stream;

struct _Stream {
    // Fields filled when the Stream is created.
    CyberKitMediaSrc* parent;

    // AppSrc. Never modified after first assignment.
    GstElement* appsrc;

    // Never modified after first assignment.
    CyberCore::SourceBufferPrivateGStreamer* sourceBuffer;

    // Fields filled when the track is attached.
    CyberCore::MediaSourceStreamTypeGStreamer type;
    GRefPtr<GstCaps> caps;

    // Only audio, video or nothing at a given time.
    RefPtr<CyberCore::AudioTrackPrivateGStreamer> audioTrack;
    RefPtr<CyberCore::VideoTrackPrivateGStreamer> videoTrack;
    CyberCore::FloatSize presentationSize;

    // This helps CyberKitMediaSrcPrivate.appsrcNeedDataCount, ensuring that needDatas are
...
enum CyberKitMediaSrcMainThreadNotification {
    ReadyForMoreSamples = 1 << 0,
    SeekNeedsData = 1 << 1
};

struct _CyberKitMediaSrcPrivate {
    ...
    CyberCore::MediaPlayerPrivateGStreamerMSE* mediaPlayerPrivate;

    RefPtr<CyberCore::MainThreadNotifier<CyberKitMediaSrcMainThreadNotification>> notifier;
...
void webKitMediaSrcDoAsyncStart(CyberKitMediaSrc*);
void webKitMediaSrcDoAsyncDone(CyberKitMediaSrc*);
GstStateChangeReturn webKitMediaSrcChangeState(GstElement*, GstStateChange);
gint64 webKitMediaSrcGetSize(CyberKitMediaSrc*);
...
void webKitMediaSrcFreeStream(CyberKitMediaSrc*, Stream*);
void webKitMediaSrcCheckAllTracksConfigured(CyberKitMediaSrc*);
