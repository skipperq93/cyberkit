...
static bool ensureGstGLContext(CyberKitGLVideoSink* sink)
{
    CyberKitGLVideoSinkPrivate* priv = sink->priv;
    ...
    if (!sharedContext) {
        GST_ELEMENT_ERROR(GST_ELEMENT_CAST(sink), RESOURCE, NOT_FOUND, (("CyberKit shared GL context unavailable")),
            ("The CyberKit shared GL context somehow disappeared. Video textures rendering will fail."));
        return false;
    }
...
GRefPtr<GstContext> requestGLContext(CyberKitGLVideoSink* sink, const char* contextType)
{
    CyberKitGLVideoSinkPrivate* priv = sink->priv;
...
{
    CyberKitGLVideoSink* sink = WEBKIT_GL_VIDEO_SINK(element);
    CyberKitGLVideoSinkPrivate* priv = sink->priv;
