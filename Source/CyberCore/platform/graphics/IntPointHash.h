...
template<> struct HashTraits<CyberCore::IntPoint> : GenericHashTraits<CyberCore::IntPoint> {
    ...
    static void constructDeletedValue(CyberCore::IntPoint& slot) { slot = CyberCore::IntPoint(std::numeric_limits<int>::min(), 0); }
    static bool isDeletedValue(const CyberCore::IntPoint& slot) { return slot == CyberCore::IntPoint(std::numeric_limits<int>::min(), 0); }
};
