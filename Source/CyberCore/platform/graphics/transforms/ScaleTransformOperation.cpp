...
Ref<TransformOperation> ScaleTransformOperation::blend(const TransformOperation* from, double progress, bool blendToIdentity)
{
    ...
    if (blendToIdentity)
        return ScaleTransformOperation::create(CyberCore::blend(m_x, 1.0, progress),
                                               CyberCore::blend(m_y, 1.0, progress),
                                               CyberCore::blend(m_z, 1.0, progress), type());
    ...
    return ScaleTransformOperation::create(CyberCore::blend(fromX, m_x, progress),
                                           CyberCore::blend(fromY, m_y, progress),
                                           CyberCore::blend(fromZ, m_z, progress), type());
}
