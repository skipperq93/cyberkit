...
typedef struct _CyberKitWebAudioSourcePrivate CyberKitWebAudioSourcePrivate;

struct _CyberKitWebAudioSrc {
    GstBin parent;

    CyberKitWebAudioSourcePrivate* priv;
};
...
#define WEBKIT_WEB_AUDIO_SRC_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), WEBKIT_TYPE_WEBAUDIO_SRC, CyberKitWebAudioSourcePrivate))
struct _CyberKitWebAudioSourcePrivate {
    gfloat sampleRate;
    AudioBus* bus;
...
static void webKitWebAudioSrcLoop(CyberKitWebAudioSrc*);
...
G_DEFINE_TYPE_WITH_CODE(CyberKitWebAudioSrc, webkit_web_audio_src, GST_TYPE_BIN, GST_DEBUG_CATEGORY_INIT(webkit_web_audio_src_debug, \
                            "webkitwebaudiosrc", \
                            0, \
                            "webaudiosrc element"));
...
static void webkit_web_audio_src_class_init(CyberKitWebAudioSrcClass* webKitWebAudioSrcClass)
{
    ...
    g_type_class_add_private(webKitWebAudioSrcClass, sizeof(CyberKitWebAudioSourcePrivate));
}

static void webkit_web_audio_src_init(CyberKitWebAudioSrc* src)
{
    CyberKitWebAudioSourcePrivate* priv = G_TYPE_INSTANCE_GET_PRIVATE(src, WEBKIT_TYPE_WEB_AUDIO_SRC, CyberKitWebAudioSourcePrivate);
    src->priv = priv;
    new (priv) CyberKitWebAudioSourcePrivate();
...
static void webKitWebAudioSrcConstructed(GObject* object)
{
    ...
    CyberKitWebAudioSourcePrivate* priv = src->priv;
...
static void webKitWebAudioSrcFinalize(GObject* object)
{
    ...
    priv->~CyberKitWebAudioSourcePrivate();
...
static Optional<Vector<GRefPtr<GstBuffer>>> webKitWebAudioSrcAllocateBuffersAndRenderAudio(CyberKitWebAudioSrc* src)
{
    CyberKitWebAudioSourcePrivate* priv = src->priv;
...
static void webKitWebAudioSrcLoop(CyberKitWebAudioSrc* src)
{
    CyberKitWebAudioSourcePrivate* priv = src->priv;
...
static GstStateChangeReturn webKitWebAudioSrcChangeState(GstElement* element, GstStateChange transition)
{
    ...
    CyberKitWebAudioSrc* src = WEBKIT_WEB_AUDIO_SRC(element);
