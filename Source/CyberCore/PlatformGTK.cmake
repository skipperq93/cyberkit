...
set(CyberCore_USER_AGENT_SCRIPTS_DEPENDENCIES ${WEBCORE_DIR}/platform/gtk/RenderThemeGtk.cpp)
...
if (ENABLE_PLUGIN_PROCESS_GTK2)
    # CyberKitPluginProcess2 needs a version of CyberCore compiled against GTK+2, so we've isolated all the GTK+
    # dependent files into a separate library which can be used to construct a GTK+2 CyberCore
    # for the plugin process.
    add_library(CyberCorePlatformGTK2 ${CyberCore_LIBRARY_TYPE} ${CyberCorePlatformGTK_SOURCES})
    add_dependencies(CyberCorePlatformGTK2 CyberCore)
    set_property(TARGET CyberCorePlatformGTK2
        APPEND
        PROPERTY COMPILE_DEFINITIONS GTK_API_VERSION_2=1
    )
    target_include_directories(CyberCorePlatformGTK2 PRIVATE
        ${CyberCore_INCLUDE_DIRECTORIES}
    )
    target_include_directories(CyberCorePlatformGTK2 SYSTEM PRIVATE
        ${CyberCore_SYSTEM_INCLUDE_DIRECTORIES}
        ${GTK2_INCLUDE_DIRS}
        ${GDK2_INCLUDE_DIRS}
    )
    target_link_libraries(CyberCorePlatformGTK2
         ${CyberCore_LIBRARIES}
         ${GTK2_LIBRARIES}
         ${GDK2_LIBRARIES}
    )
...
add_library(CyberCorePlatformGTK ${CyberCore_LIBRARY_TYPE} ${CyberCorePlatformGTK_SOURCES})
add_dependencies(CyberCorePlatformGTK CyberCore)
target_include_directories(CyberCorePlatformGTK PRIVATE
    ${CyberCore_INCLUDE_DIRECTORIES}
)
target_include_directories(CyberCorePlatformGTK SYSTEM PRIVATE
    ${CyberCore_SYSTEM_INCLUDE_DIRECTORIES}
    ${GTK_INCLUDE_DIRS}
    ${GDK_INCLUDE_DIRS}
)
target_link_libraries(CyberCorePlatformGTK
    ${CyberCore_LIBRARIES}
    ${GTK_LIBRARIES}
    ${GDK_LIBRARIES}
)
