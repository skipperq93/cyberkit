macro(MAKE_HASH_TOOLS _source)
    ...
    if (${_source} STREQUAL "DocTypeStrings")
        set(_hash_tools_h "${CyberCore_DERIVED_SOURCES_DIR}/HashTools.h")
    else ()
        set(_hash_tools_h "")
    endif ()

    add_custom_command(
        OUTPUT ${CyberCore_DERIVED_SOURCES_DIR}/${_name}.cpp ${_hash_tools_h}
        MAIN_DEPENDENCY ${_source}.gperf
        COMMAND ${PERL_EXECUTABLE} ${WEBCORE_DIR}/make-hash-tools.pl ${CyberCore_DERIVED_SOURCES_DIR} ${_source}.gperf ${GPERF_EXECUTABLE}
        VERBATIM)
...
# This one consider the given dependencies are in ${CyberCore_DERIVED_SOURCES_DIR}
# and prepends this to every member of dependencies list
macro(ADD_SOURCE_WEBCORE_DERIVED_DEPENDENCIES _source _deps)
    set(_tmp "")
    foreach (f ${_deps})
        list(APPEND _tmp "${CyberCore_DERIVED_SOURCES_DIR}/${f}")
    endforeach ()
...
macro(MAKE_JS_FILE_ARRAYS _output_cpp _output_h _namespace _scripts _scripts_dependencies)
    add_custom_command(
        ...
        DEPENDS ${CyberScriptCore_SCRIPTS_DIR}/make-js-file-arrays.py ${${_scripts}}
        COMMAND ${PYTHON_EXECUTABLE} ${CyberScriptCore_SCRIPTS_DIR}/make-js-file-arrays.py -n ${_namespace} ${_output_h} ${_output_cpp} ${${_scripts}}
        VERBATIM)
...
#   OUTPUT_SOURCE is a list name which will contain generated sources.(eg. CyberCore_SOURCES)
...
macro(GENERATE_FONT_NAMES _infile)
    ...
    set(_outputfiles ${CyberCore_DERIVED_SOURCES_DIR}/CyberKitFontFamilyNames.cpp ${CyberCore_DERIVED_SOURCES_DIR}/CyberKitFontFamilyNames.h)

    add_custom_command(
        OUTPUT  ${_outputfiles}
        MAIN_DEPENDENCY ${_infile}
        DEPENDS ${MAKE_NAMES_DEPENDENCIES} ${NAMES_GENERATOR} ${SCRIPTS_BINDINGS}
        COMMAND ${PERL_EXECUTABLE} ${NAMES_GENERATOR} --outputDir ${CyberCore_DERIVED_SOURCES_DIR} ${_arguments}
        VERBATIM)
endmacro()
...
macro(GENERATE_EVENT_FACTORY _infile _namespace)
    ...
    set(_outputfiles ${CyberCore_DERIVED_SOURCES_DIR}/${_namespace}Interfaces.h ${CyberCore_DERIVED_SOURCES_DIR}/${_namespace}Factory.cpp)

    add_custom_command(
        OUTPUT  ${_outputfiles}
        MAIN_DEPENDENCY ${_infile}
        DEPENDS ${NAMES_GENERATOR} ${SCRIPTS_BINDINGS}
        COMMAND ${PERL_EXECUTABLE} ${NAMES_GENERATOR} --input ${_infile} --outputDir ${CyberCore_DERIVED_SOURCES_DIR}
        VERBATIM)
endmacro()
...
macro(GENERATE_SETTINGS_MACROS _infile _outfile)
    ...
    set(_extra_output
        ${CyberCore_DERIVED_SOURCES_DIR}/Settings.cpp
        ${CyberCore_DERIVED_SOURCES_DIR}/InternalSettingsGenerated.h
        ${CyberCore_DERIVED_SOURCES_DIR}/InternalSettingsGenerated.cpp
        ${CyberCore_DERIVED_SOURCES_DIR}/InternalSettingsGenerated.idl
    )
    ...
    add_custom_command(
        OUTPUT ${CyberCore_DERIVED_SOURCES_DIR}/${_outfile} ${_extra_output}
        MAIN_DEPENDENCY ${_infile}
        DEPENDS ${NAMES_GENERATOR} ${GENERATE_SETTINGS_SCRIPTS} ${SCRIPTS_BINDINGS}
        COMMAND ${RUBY_EXECUTABLE} ${NAMES_GENERATOR} --input ${_infile} --outputDir ${CyberCore_DERIVED_SOURCES_DIR}
        VERBATIM ${_args})
endmacro()
...
macro(GENERATE_DOM_NAMES _namespace _attrs)
    ...
    set(_outputfiles ${CyberCore_DERIVED_SOURCES_DIR}/${_namespace}Names.cpp ${CyberCore_DERIVED_SOURCES_DIR}/${_namespace}Names.h)
    set(_extradef)
    set(_tags)
    ...
    if (_tags)
        ...
        set(_outputfiles "${_outputfiles}" ${CyberCore_DERIVED_SOURCES_DIR}/${_namespace}ElementFactory.cpp ${CyberCore_DERIVED_SOURCES_DIR}/${_namespace}ElementFactory.h ${CyberCore_DERIVED_SOURCES_DIR}/${_namespace}ElementTypeHelpers.h ${CyberCore_DERIVED_SOURCES_DIR}/JS${_namespace}ElementWrapperFactory.cpp ${CyberCore_DERIVED_SOURCES_DIR}/JS${_namespace}ElementWrapperFactory.h)
    endif ()
    ...
    add_custom_command(
        ...
        COMMAND ${PERL_EXECUTABLE} ${NAMES_GENERATOR} --preprocessor "${CODE_GENERATOR_PREPROCESSOR_WITH_LINEMARKERS}" --outputDir ${CyberCore_DERIVED_SOURCES_DIR} ${_arguments} ${_additionArguments}
        VERBATIM)
endmacro()
