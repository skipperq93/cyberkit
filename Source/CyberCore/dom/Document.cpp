...
static bool isAttributeOnAllOwners(const CyberCore::QualifiedName& attribute, const CyberCore::QualifiedName& prefixedAttribute, const HTMLFrameOwnerElement* owner)
{
    if (!owner)
        return true;
    do {
...
void Document::processHttpEquiv(const String& equiv, const String& content, bool isInDocumentHead)
{
    ...
    case HTTPHeaderName::XCyberKitCSP:
        if (isInDocumentHead)
