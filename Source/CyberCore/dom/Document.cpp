...
void Document::loadEventDelayTimerFired()
{
    ...
    // visible to CyberKit clients, but it's more like a race than a well-defined relationship.
    Ref<Document> protectedThis(*this);
    checkCompleted();
    if (RefPtr frame = this->frame())
...
void Document::updateSleepDisablerIfNeeded()
{
    ...
    if (m_mediaState & activeVideoCaptureMask) {
        m_sleepDisabler = makeUnique<SleepDisabler>("com.matthewbenedict.CyberCore: Document doing camera, screen or window capture"_s, PAL::SleepDisabler::Type::Display, pageID());
        return;
    }
...
#if __has_include(<CyberKitAdditions/DocumentAdditions.cpp>)
#include <CyberKitAdditions/DocumentAdditions.cpp>
#else
