...
@interface CyberCoreTextFieldCell : NSTextFieldCell
@end

@implementation CyberCoreTextFieldCell
...
String RenderThemeMac::mediaControlsStyleSheet()
{
    ...
    if (m_legacyMediaControlsStyleSheet.isEmpty())
        m_legacyMediaControlsStyleSheet = [NSString stringWithContentsOfFile:[[NSBundle bundleForClass:[CyberCoreRenderThemeBundle class]] pathForResource:@"mediaControlsApple" ofType:@"css"] encoding:NSUTF8StringEncoding error:nil];
...
String RenderThemeMac::modernMediaControlsStyleSheet()
{
    ...
    if (RuntimeEnabledFeatures::sharedFeatures().modernMediaControlsEnabled()) {
        if (m_mediaControlsStyleSheet.isEmpty())
            m_mediaControlsStyleSheet = [NSString stringWithContentsOfFile:[[NSBundle bundleForClass:[CyberCoreRenderThemeBundle class]] pathForResource:@"modern-media-controls" ofType:@"css" inDirectory:@"modern-media-controls"] encoding:NSUTF8StringEncoding error:nil];
...
{
    // The inactive color is normally used, since no legacy CyberKit client marks a text match as active.
...
{
    if (!m_textField) {
        m_textField = adoptNS([[CyberCoreTextFieldCell alloc] initTextCell:@""]);
