...
#include "CyberKitAccessibleHyperlink.h"
#include "CyberKitAccessibleInterfaceAction.h"
#include "CyberKitAccessibleInterfaceComponent.h"
#include "CyberKitAccessibleInterfaceDocument.h"
#include "CyberKitAccessibleInterfaceEditableText.h"
#include "CyberKitAccessibleInterfaceHyperlinkImpl.h"
#include "CyberKitAccessibleInterfaceHypertext.h"
#include "CyberKitAccessibleInterfaceImage.h"
#include "CyberKitAccessibleInterfaceSelection.h"
#include "CyberKitAccessibleInterfaceTable.h"
#include "CyberKitAccessibleInterfaceTableCell.h"
#include "CyberKitAccessibleInterfaceText.h"
#include "CyberKitAccessibleInterfaceValue.h"
#include "CyberKitAccessibleUtil.h"
...
struct _CyberKitAccessiblePrivate {
...
WEBKIT_DEFINE_TYPE(CyberKitAccessible, webkit_accessible, ATK_TYPE_OBJECT)
...
static const gchar* webkitAccessibleGetName(AtkObject* object)
{
    ...
    returnValIfCyberKitAccessibleIsInvalid(accessible, nullptr);
...
{
    // The root accessible object in CyberCore is always an object with
...
{
    auto* accessible = WEBKIT_ACCESSIBLE(object);
    returnValIfCyberKitAccessibleIsInvalid(accessible, 0);
...
{
    auto* accessible = WEBKIT_ACCESSIBLE(object);
    returnValIfCyberKitAccessibleIsInvalid(accessible, -1);
...
#if PLATFORM(GTK)
    attributeSet = addToAtkAttributeSet(attributeSet, "toolkit", "CyberKitGtk");
#elif PLATFORM(WPE)
    attributeSet = addToAtkAttributeSet(attributeSet, "toolkit", "WPECyberKit");
#endif

    auto* coreObject = accessible->priv->object;

    // Hack needed for CyberKit2 tests because obtaining an element by its ID
...
static AtkRole webkitAccessibleGetRole(AtkObject* object)
{
    ...
    // CyberCore accessible object for which the platform role mapping is unknown.
    auto* accessible = WEBKIT_ACCESSIBLE(object);
    returnValIfCyberKitAccessibleIsInvalid(accessible, ATK_ROLE_INVALID);

    // Note: Why doesn't CyberCore have a password field for this
...
static void setAtkStateSetFromCoreObject(AccessibilityObject* coreObject, AtkStateSet* stateSet)
{
    ...
    // Not sure how to handle this in CyberKit, see bug
...
static void webkitAccessibleInit(AtkObject* object, gpointer data)
{
    ...
    CyberKitAccessible* accessible = WEBKIT_ACCESSIBLE(object);
...
static void webkit_accessible_class_init(CyberKitAccessibleClass* klass)
{
...
static guint16 interfaceMaskFromObject(AXCoreObject* coreObject)
{
    ...
    // the CyberKitAccessible class and let CyberCore decide what to do.
...
CyberKitAccessible* webkitAccessibleNew(AccessibilityObject* coreObject)
{
...
AccessibilityObject& webkitAccessibleGetAccessibilityObject(CyberKitAccessible* accessible)
{
    ASSERT(WEBKIT_IS_ACCESSIBLE(accessible));
    return *accessible->priv->object;
}

void webkitAccessibleDetach(CyberKitAccessible* accessible)
{
    ASSERT(WEBKIT_IS_ACCESSIBLE(accessible));
    ...
    // We replace the CyberCore AccessibilityObject with a fallback object that
    // provides default implementations to avoid repetitive null-checking after
    // detachment.
    accessible->priv->object = fallbackObject();
}

bool webkitAccessibleIsDetached(CyberKitAccessible* accessible)
{
    ASSERT(WEBKIT_IS_ACCESSIBLE(accessible));
    return accessible->priv->object == fallbackObject();
}

const char* webkitAccessibleCacheAndReturnAtkProperty(CyberKitAccessible* accessible, AtkCachedProperty property, CString&& value)
{
    ASSERT(WEBKIT_IS_ACCESSIBLE(accessible));

    CyberKitAccessiblePrivate* priv = accessible->priv;
