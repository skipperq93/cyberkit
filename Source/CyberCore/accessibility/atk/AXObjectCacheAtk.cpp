...
static AtkObject* wrapperParent(CyberKitAccessible* wrapper)
{
    ...
    // since the accessibility hierarchy in CyberCore will no longer be navigable.
    ...
    // We don't want to emit any signal from an object outside CyberKit's world.
...
void AXObjectCache::attachWrapper(AXCoreObject* obj)
{
    ...
    GRefPtr<CyberKitAccessible> wrapper = adoptGRef(webkitAccessibleNew(accessibilityObject));
...
static void notifyChildrenSelectionChange(AXCoreObject* object)
{
    ...
    CyberKitAccessible* axItem = item ? item->wrapper() : nullptr;
    CyberKitAccessible* axOldFocusedObject = oldFocusedObject.get() ? oldFocusedObject.get()->wrapper() : nullptr;
