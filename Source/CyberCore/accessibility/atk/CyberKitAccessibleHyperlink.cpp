...
struct _CyberKitAccessibleHyperlinkPrivate {
    CyberKitAccessible* hyperlinkImpl;
...
#define WEBKIT_ACCESSIBLE_HYPERLINK_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), WEBKIT_TYPE_ACCESSIBLE_HYPERLINK, CyberKitAccessibleHyperlinkPrivate))
...
static AccessibilityObject* core(CyberKitAccessible* accessible)
{
...
static AccessibilityObject* core(CyberKitAccessibleHyperlink* link)
{
    if (!link)
        return 0;
...
static gboolean webkitAccessibleHyperlinkActionDoAction(AtkAction* action, gint index)
{
    ...
    returnValIfCyberKitAccessibleIsInvalid(WEBKIT_ACCESSIBLE_HYPERLINK(action)->priv->hyperlinkImpl, FALSE);
...
static gint webkitAccessibleHyperlinkActionGetNActions(AtkAction* action)
{
    ...
    returnValIfCyberKitAccessibleIsInvalid(WEBKIT_ACCESSIBLE_HYPERLINK(action)->priv->hyperlinkImpl, 0);
...
static const gchar* webkitAccessibleHyperlinkActionGetKeybinding(AtkAction* action, gint index)
{
    ...
    CyberKitAccessibleHyperlinkPrivate* priv = WEBKIT_ACCESSIBLE_HYPERLINK(action)->priv;
    returnValIfCyberKitAccessibleIsInvalid(priv->hyperlinkImpl, 0);
...
static gchar* webkitAccessibleHyperlinkGetURI(AtkHyperlink* link, gint index)
{
    ...
    returnValIfCyberKitAccessibleIsInvalid(WEBKIT_ACCESSIBLE_HYPERLINK(link)->priv->hyperlinkImpl, 0);
...
static gboolean webkitAccessibleHyperlinkIsValid(AtkHyperlink* link)
{
    ...
    returnValIfCyberKitAccessibleIsInvalid(WEBKIT_ACCESSIBLE_HYPERLINK(link)->priv->hyperlinkImpl, FALSE);
...
{
    CyberKitAccessibleHyperlinkPrivate* priv = WEBKIT_ACCESSIBLE_HYPERLINK(object)->priv;
...
static void webkitAccessibleHyperlinkClassInit(AtkHyperlinkClass* klass)
{
    ...
    g_object_class_install_property(gobjectClass, PROP_HYPERLINK_IMPL,
        g_param_spec_object("hyperlink-impl",
            ...
            "The associated CyberKitAccessible instance.",
            WEBKIT_TYPE_ACCESSIBLE,
            (GParamFlags)(G_PARAM_READWRITE | G_PARAM_CONSTRUCT_ONLY | G_PARAM_STATIC_STRINGS)));

    g_type_class_add_private(gobjectClass, sizeof(CyberKitAccessibleHyperlinkPrivate));
}
...
GType webkitAccessibleHyperlinkGetType()
{
    ...
    if (g_once_init_enter(&typeVolatile)) {
        static const GTypeInfo tinfo = {
            sizeof(CyberKitAccessibleHyperlinkClass),
            (GBaseInitFunc) 0,
            (GBaseFinalizeFunc) 0,
            (GClassInitFunc)(GCallback) webkitAccessibleHyperlinkClassInit,
            (GClassFinalizeFunc) 0,
            0, /* class data */
            sizeof(CyberKitAccessibleHyperlink), /* instance size */
            0, /* nb preallocs */
        ...
        GType type = g_type_register_static(ATK_TYPE_HYPERLINK, "CyberKitAccessibleHyperlink", &tinfo, GTypeFlags(0));
...
CyberKitAccessibleHyperlink* webkitAccessibleHyperlinkNew(AtkHyperlinkImpl* hyperlinkImpl)
{
...
CyberCore::AccessibilityObject* webkitAccessibleHyperlinkGetAccessibilityObject(CyberKitAccessibleHyperlink* link)
{
    g_return_val_if_fail(WEBKIT_IS_ACCESSIBLE_HYPERLINK(link), 0);
    return core(link);
}
