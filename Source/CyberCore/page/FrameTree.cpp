...
static void printFrames(const CyberCore::Frame& frame, const CyberCore::Frame* targetFrame, int indent)
{
    ...
    for (CyberCore::Frame* child = frame.tree().firstChild(); child; child = child->tree().nextSibling())
        printFrames(*child, targetFrame, indent + 1);
}

void showFrameTree(const CyberCore::Frame* frame)
{
    if (!frame) {
