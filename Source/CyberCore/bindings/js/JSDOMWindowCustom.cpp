...
#if ENABLE(USER_MESSAGE_HANDLERS)
static EncodedJSValue jsDOMWindowCyberKit(JSGlobalObject* lexicalGlobalObject, EncodedJSValue thisValue, PropertyName)
{
...
#if ENABLE(USER_MESSAGE_HANDLERS)
    if (propertyName == static_cast<JSVMClientData*>(lexicalGlobalObject->vm().clientData)->builtinNames().webkitPublicName() && thisObject->wrapped().shouldHaveCyberKitNamespaceForWorld(thisObject->world())) {
