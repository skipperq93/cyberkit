...
sub GenerateHeader
{
    ...
    # FIXME: We put this unconditionally to put all the CyberCore JS wrappers in each IsoSubspace.
...
sub GenerateOverloadDispatcher
{
    ...
    for my $length ( sort keys %allSets ) {
        ...
        if (scalar(@$S) > 1) {
            ...
            &$generateOverloadCallIfNecessary($overload, "hasIteratorMethod(lexicalGlobalObject, distinguishingArg)", "<CyberScriptCore/IteratorOperations.h>");
    ...
    if (ShouldGenerateWrapperOwnerCode($hasParent, $interface) && !GetCustomIsReachable($interface)) {
        ...
        if (GetGenerateIsReachable($interface)) {
            ...
            if (GetGenerateIsReachable($interface) eq "Impl") {
                ...
                $rootString .= "    void* root = CyberCore::root(element);\n";
            } elsif (GetGenerateIsReachable($interface) eq "ImplOwnerNodeRoot") {
                $implIncludes{"Element.h"} = 1;
                $implIncludes{"JSNodeCustom.h"} = 1;
                $rootString  = "    void* root = CyberCore::root(js${interfaceName}->wrapped().ownerNode());\n";
                $rootString .= "    if (UNLIKELY(reason))\n";
                ...
                $rootString  = "    void* root = CyberCore::root(&js${interfaceName}->wrapped());\n";
                $rootString .= "    if (UNLIKELY(reason))\n";
