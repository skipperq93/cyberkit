...
SPECIALIZE_TYPE_TRAITS_BEGIN(CyberCore::StyleRule)
    static bool isType(const CyberCore::StyleRuleBase& rule) { return rule.isStyleRule(); }
...
#if ENABLE(CSS_DEVICE_ADAPTATION)
SPECIALIZE_TYPE_TRAITS_BEGIN(CyberCore::StyleRuleViewport)
    static bool isType(const CyberCore::StyleRuleBase& rule) { return rule.isViewportRule(); }
SPECIALIZE_TYPE_TRAITS_END()
#endif // ENABLE(CSS_DEVICE_ADAPTATION)
