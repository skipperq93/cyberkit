                ...
                for(;;) {
                    if((cq=*q)==0) {
                        if(isMatchAtCPBoundary(start, s-1, p, nullptr)) {
                            return const_cast<char16_t *>(s-1); /* well-formed match */
                        } else {
...
u_strchr32(const char16_t *s, UChar32 c) {
    if((uint32_t)c<=U_BMP_MAX) {
        ...
        while((cs=*s++)!=0) {
            if(cs==lead && *s==trail) {
                return const_cast<char16_t *>(s-1);
            }
        }
        return nullptr;
    } else {
            ...
            for(;;) {
                if(q==sub) {
                    if(isMatchAtCPBoundary(start, p, limit+1, start+length)) {
                        return const_cast<char16_t *>(p); /* well-formed match */
                    } else {
                        break; /* no match because surrogate pair is split */
                    }
                }
...
u_strrchr(const char16_t *s, char16_t c) {
    if(U16_IS_SURROGATE(c)) {
        ...
        for(;;) {
            ...
            if(cs==0) {
                return const_cast<char16_t *>(result);
            }
            ++s;
        }
    }
...
u_memrchr(const char16_t *s, char16_t c, int32_t count) {
    if(count<=0) {
        ...
        do {
            if(*(--limit)==c) {
                return const_cast<char16_t *>(limit);
            }
        } while(s!=limit);
        return nullptr;
    }
    ...
    if((uint32_t)c<=U_BMP_MAX) {
        ...
        do {
            if(*limit==trail && *(limit-1)==lead) {
                return const_cast<char16_t *>(limit-1);
            }
        } while(s!=--limit);
        return nullptr;
    } else {
...
u_strpbrk(const char16_t *string, const char16_t *matchSet)
{
    ...
    if(idx >= 0) {
        return const_cast<char16_t *>(string) + idx;
    } else {
        return nullptr;
    }
}
...
u_strCompareIter(UCharIterator *iter1, UCharIterator *iter2, UBool codePointOrder) {
    ...
    if(c1>=0xd800 && c2>=0xd800 && codePointOrder) {
        ...
        if(
           ...
           (U16_IS_TRAIL(c1) && (static_cast<void>(iter1->previous(iter1)), U16_IS_LEAD(iter1->previous(iter1))))
        ) {
            /* part of a surrogate pair, leave >=d800 */
        } else {
            /* BMP code point - may be surrogate code point - make <d800 */
        ...
        if(
           ...
           (U16_IS_TRAIL(c2) && (static_cast<void>(iter2->previous(iter2)), U16_IS_LEAD(iter2->previous(iter2))))
        ) {
            /* part of a surrogate pair, leave >=d800 */
        } else {
            /* BMP code point - may be surrogate code point - make <d800 */
