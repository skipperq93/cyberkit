...
void RBBIDataWrapper::init(const RBBIDataHeader *data, UErrorCode &status) {
    ...
    if (data->fFTableLen != 0) {
        fForwardTable = (RBBIStateTable *)(const_cast<char *>((const char*)data) + fHeader->fFTable);
    }
    if (data->fRTableLen != 0) {
        fReverseTable = (RBBIStateTable *)(const_cast<char *>((const char*)data) + fHeader->fRTable);
    }

    fTrie = ucptrie_openFromBinary(UCPTRIE_TYPE_FAST,
                                   UCPTRIE_VALUE_BITS_ANY,
                                   const_cast<uint8_t *>((const uint8_t*)data) + fHeader->fTrie,
                                   fHeader->fTrieLen,
    ...
    fRuleSource   = (const_cast<char *>((const char*)data) + fHeader->fRuleSource);
    fRuleString = UnicodeString::fromUTF8(StringPiece(fRuleSource, fHeader->fRuleSourceLen));
    U_ASSERT(data->fRuleSourceLen > 0);

    fRuleStatusTable = (int32_t *)(const_cast<char *>((const char*)data) + fHeader->fStatusTable);
...
ubrk_swap(const UDataSwapper *ds, const void *inData, int32_t length, void *outData,
    ...
    RBBIDataHeader *rbbiDH = const_cast<RBBIDataHeader*>((const RBBIDataHeader *)inBytes);
    ...
    if (tableLength > 0) {
        RBBIStateTable *rbbiST = const_cast<RBBIStateTable*>((const RBBIStateTable *)(inBytes+tableStartOffset));
