...
swapFormatVersion3(const UDataSwapper *ds,
    ...
    if(length>=0) {
        ...
        if(header.scriptToLeadByte!=0) {
            int indexCount = ds->readUInt16(*((const uint16_t*)(inBytes+header.scriptToLeadByte))); // each entry = 2 * uint16
            int dataCount = ds->readUInt16(*((const uint16_t*)(inBytes+header.scriptToLeadByte + 2))); // each entry = uint16
        ...
        if(header.leadByteToScript!=0) {
            int indexCount = ds->readUInt16(*((const uint16_t*)(inBytes+header.leadByteToScript))); // each entry = uint16
            int dataCount = ds->readUInt16(*((const uint16_t*)(inBytes+header.leadByteToScript + 2))); // each entry = uint16
