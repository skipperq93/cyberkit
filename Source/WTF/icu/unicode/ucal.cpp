           ...
           UErrorCode*      status)
{
  ...
  Calendar* res = ((const Calendar*)cal)->clone();
                   ...
                   UErrorCode *status)
{
    ...
    const TimeZone& tz = ((const Calendar*)cal)->getTimeZone();
    UnicodeString id;
    tz.getID(id);
                    ...
                    UErrorCode*     status )
{
    ...
    return ((const Calendar*)cal)->inDaylightTime(*status);
}
...
ucal_getAttribute(    const    UCalendar*              cal,
    ...
    switch(attr) {
  case UCAL_LENIENT:
      return ((const Calendar*)cal)->isLenient();

  case UCAL_FIRST_DAY_OF_WEEK:
      return ((const Calendar*)cal)->getFirstDayOfWeek();

  case UCAL_MINIMAL_DAYS_IN_FIRST_WEEK:
      return ((const Calendar*)cal)->getMinimalDaysInFirstWeek();

  case UCAL_REPEATED_WALL_TIME:
      return ((const Calendar*)cal)->getRepeatedWallTimeOption();

  case UCAL_SKIPPED_WALL_TIME:
      return ((const Calendar*)cal)->getSkippedWallTimeOption();
               ...
               UErrorCode*     status)
{
    ...
    return ((const Calendar*)cal)->getTime(*status);
}
                  ...
                  const UCalendar*      cal2)
{
    ...
    return ((const Calendar*)cal1)->isEquivalentTo(*((const Calendar*)cal2));
}
...
ucal_get(    const    UCalendar*                cal,
    ...
    return ((const Calendar*)cal)->get(field, *status);
}
...
ucal_isSet(    const    UCalendar*                cal,
    ...
    return ((const Calendar*)cal)->isSet(field);
}
...
ucal_getLimit(    const    UCalendar*              cal,
    ...
    switch(type) {
  case UCAL_MINIMUM:
      return ((const Calendar*)cal)->getMinimum(field);

  case UCAL_MAXIMUM:
      return ((const Calendar*)cal)->getMaximum(field);

  case UCAL_GREATEST_MINIMUM:
      return ((const Calendar*)cal)->getGreatestMinimum(field);

  case UCAL_LEAST_MAXIMUM:
      return ((const Calendar*)cal)->getLeastMaximum(field);

  case UCAL_ACTUAL_MINIMUM:
      return ((const Calendar*)cal)->getActualMinimum(field,
          *status);

  case UCAL_ACTUAL_MAXIMUM:
      return ((const Calendar*)cal)->getActualMaximum(field,
          *status);
...
ucal_getLocaleByType(const UCalendar *cal, ULocDataLocaleType type, UErrorCode* status) 
{
    ...
    return ((const Calendar*)cal)->getLocaleID(type, *status);
}
...
ucal_getType(const UCalendar *cal, UErrorCode* status)
{
    ...
    return ((const Calendar*)cal)->getType();
}
...
ucal_getDayOfWeekType(const UCalendar *cal, UCalendarDaysOfWeek dayOfWeek, UErrorCode* status)
{
    ...
    return ((const Calendar*)cal)->getDayOfWeekType(dayOfWeek, *status);
}
...
ucal_getWeekendTransition(const UCalendar *cal, UCalendarDaysOfWeek dayOfWeek, UErrorCode *status)
{
    ...
    return ((const Calendar*)cal)->getWeekendTransition(dayOfWeek, *status);
}
...
ucal_isWeekend(const UCalendar *cal, UDate date, UErrorCode *status)
{
    ...
    return ((const Calendar*)cal)->isWeekend(date, *status);
}
                               ...
                               UDate* transition, UErrorCode* status)
{
    ...
    UDate base = ((const Calendar*)cal)->getTime(*status);
    const TimeZone& tz = ((const Calendar*)cal)->getTimeZone();
    ...
    int32_t* rawOffset, int32_t* dstOffset, UErrorCode* status)
{
    ...
    UDate date = ((const Calendar*)cal)->getTime(*status);
    if (U_FAILURE(*status)) {
        return;
    }
    const TimeZone& tz = ((const Calendar*)cal)->getTimeZone();
