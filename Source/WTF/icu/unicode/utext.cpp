...
static void adjustPointer(UText *dest, const void **destPtr, const UText *src) {
    ...
    const char  *dptr = (const char *)*destPtr;
    const char  *dUText = (const char *)dest;
    const char  *sUText = (const char *)src;
...
utf8TextAccess(UText *ut, int64_t index, UBool forward) {
    ...
    if (forward) {
        if (ix==ut->chunkNativeLimit) {
            ...
            if (ix==length) {
                ...
                const UTF8Buf *altB = (const UTF8Buf *)ut->q;
        ...
        {
            u8b = const_cast<UTF8Buf *>((const UTF8Buf *)ut->q);   // the alternate buffer
            ...
            u8b = const_cast<UTF8Buf*>((const UTF8Buf *)ut->p);   // the current buffer
    ...
    u8b = const_cast<UTF8Buf *>((const UTF8Buf *)ut->p);
    mapIndex = ix - u8b->toUCharsMapStart;
    ut->chunkOffset = u8b->mapToUChars[mapIndex] - u8b->bufStartIdx;
    if (ut->chunkOffset==0) {
...
swapBuffers:
    ...
    {
        u8b   = const_cast<UTF8Buf*>((const UTF8Buf *)ut->q);
        ut->q = ut->p;
        ut->p = u8b;
...
fillForward:
    {
        ...
        UTF8Buf *u8b_swap = const_cast<UTF8Buf*>((const UTF8Buf *)ut->q);
        ut->q = ut->p;
        ut->p = u8b_swap;
        ...
        char16_t   *buf = const_cast<char16_t*>(u8b_swap->buf);
        uint8_t *mapToNative  = const_cast<uint8_t*>(u8b_swap->mapToNative);
        uint8_t *mapToUChars  = const_cast<uint8_t*>(u8b_swap->mapToUChars);
        int32_t  destIx       = 0;
        int32_t  srcIx        = ix;
              ...
              )
{
    ...
    int32_t index = 0;
    int32_t reqLength = 0;
    uint8_t* pSrc = const_cast<uint8_t*>((const uint8_t*)src);
...
utf8TextMapOffsetToNative(const UText *ut) {
    //
    UTF8Buf *u8b = const_cast<UTF8Buf *>((const UTF8Buf*) ut->p);
...
utf8TextClone(UText *dest, const UText *src, UBool deep, UErrorCode *status)
{
    ...
    if (deep && U_SUCCESS(*status)) {
        int32_t  len = (int32_t)utext_nativeLength(const_cast<UText*>((const UText *)src));
...
utf8TextClose(UText *ut) {
    ...
    if (ut->providerProperties & I32_FLAG(UTEXT_PROVIDER_OWNS_TEXT)) {
        char *s = const_cast<char*>((const char *)ut->context);
        uprv_free(s);
        ut->context = nullptr;
    }
}
...
repTextClose(UText *ut) {
    ...
    //  owns it.  This occurs if the UText was created by cloning.
    if (ut->providerProperties & I32_FLAG(UTEXT_PROVIDER_OWNS_TEXT)) {
        Replaceable *rep = const_cast<Replaceable*>((const Replaceable *)ut->context);
        delete rep;
        ut->context = nullptr;
    }
}
...
unistrTextClose(UText *ut) {
    ...
    //  owns it.  This occurs if the UText was created by cloning.
    if (ut->providerProperties & I32_FLAG(UTEXT_PROVIDER_OWNS_TEXT)) {
        UnicodeString *str = const_cast<UnicodeString*>((const UnicodeString *)ut->context);
        delete str;
        ut->context = nullptr;
    }
}
...
unistrTextReplace(UText *ut,
                  ...
                  const char16_t *src, int32_t length,
                  UErrorCode *pErrorCode) {
    UnicodeString *us=const_cast<UnicodeString*>((const UnicodeString *)ut->context);
    int32_t oldLength;
...
ucstrTextClose(UText *ut) {
    ...
    //  owns it.  This occurs if the UText was created by cloning.
    if (ut->providerProperties & I32_FLAG(UTEXT_PROVIDER_OWNS_TEXT)) {
        char16_t *s = const_cast<char16_t*>((const char16_t *)ut->context);
        uprv_free(s);
        ut->context = nullptr;
    }
}
...
charIterTextClose(UText *ut) {
    ...
    //  owns it.  This occurs if the UText was created by cloning.
    CharacterIterator *ci = const_cast<CharacterIterator*>((const CharacterIterator *)ut->r);
    delete ci;
    ut->r = nullptr;
}
...
charIterTextAccess(UText *ut, int64_t index, UBool  forward) {
    CharacterIterator *ci   = const_cast<CharacterIterator*>((const CharacterIterator *)ut->context);
    ...
    if (ut->chunkNativeStart == neededIndex) {
        ...
        buf = const_cast<char16_t*>((const char16_t *)ut->p);
    } else if (ut->c == neededIndex) {
        // The second buffer (buffer q) has what we need.
        buf = const_cast<char16_t*>((const char16_t *)ut->q);
    } else {
...
charIterTextClone(UText *dest, const UText *src, UBool deep, UErrorCode * status) {
    ...
    if (deep) {
        ...
        *status = U_UNSUPPORTED_ERROR;
        ...
        CharacterIterator *srcCI = const_cast<CharacterIterator*>((const CharacterIterator *)src->context);
        srcCI = srcCI->clone();
...
utext_openCharacterIterator(UText *ut, CharacterIterator *ci, UErrorCode *status) {
    ...
    if (U_SUCCESS(*status)) {
        ...
        ut->chunkContents        = const_cast<char16_t*>((const char16_t *)ut->p);
