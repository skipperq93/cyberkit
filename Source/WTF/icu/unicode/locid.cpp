...
Locale *locale_set_default_internal(const char *id, UErrorCode& status) {
    ...
    if (newDefault == nullptr) {
        ...
        uhash_put(gDefaultLocalesHashT, const_cast<char*>((const char*) newDefault->getName()), newDefault, &status);
        if (U_FAILURE(status)) {
            return gDefaultLocale;
        }
    }
    ...
    CharString& transformedExtensions, CharString& output, UErrorCode& status)
{
    ...
    if (tkey != nullptr) {
        ...
        do {
            ...
            if (nextTKey != nullptr) {
                *(const_cast<char*>((const char*)(nextTKey-1))) = '\0';  // NUL terminate tvalue
            }
        ...
        for (int32_t i = 0; i < tfields.size(); i++) {
             ...
             *(const_cast<char*>((const char*)tvalue++)) = '\0'; // NUL terminate tkey
...
public:
    KeywordEnumeration(const char *keys, int32_t keywordLen, int32_t currentIndex, UErrorCode &status)
        : keywords(const_cast<char*>((const char *)&fgClassID)), current(const_cast<char*>((const char *)&fgClassID)), length(0) {
