...
ucnv_safeClone(const UConverter* cnv, void *stackBuffer, int32_t *pBufferSize, UErrorCode *status)
{
    ...
    if (cnv->subChars == (const uint8_t *)cnv->subUChars) {
...
#pragma clang diagnostic pop
    ...
    if (cnv->sharedData->impl->writeSub == nullptr
        ...
        subChars = const_cast<uint8_t*>((const uint8_t *)s);
        if (length < 0) {
            length = u_strlen(s);
        }
...
ucnv_convertEx(UConverter *targetCnv, UConverter *sourceCnv,
    ...
    if(pivotStart==nullptr) {
        ...
        pivotSource=const_cast<char16_t **>(&myPivotSource);
    ...
    for(;;) {
        ...
        if( *pivotSource<*pivotTarget ||
            ...
            if(U_FAILURE(*pErrorCode)) {
                ...
                *pivotSource=const_cast<char16_t *>(fromUArgs.source);
                break;
            }
