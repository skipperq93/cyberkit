...
ZNameSearchHandler::handleMatch(int32_t matchLength, const CharacterNode *node, UErrorCode &status) {
    ...
    if (node->hasValues()) {
        ...
        for (int32_t i = 0; i < valuesCount; i++) {
            ZNameInfo *nameinfo = const_cast<ZNameInfo*>((const ZNameInfo *)node->getValue(i));
            if (nameinfo == nullptr) {
                continue;
            }
...
TZDBNames::getParseRegions(int32_t& numRegions) const {
    ...
    return const_cast<const char**>((const char* const*)fRegions);
}
...
TZDBNameSearchHandler::handleMatch(int32_t matchLength, const CharacterNode *node, UErrorCode &status) {
    ...
    if (node->hasValues()) {
        int32_t valuesCount = node->countValues();
        for (int32_t i = 0; i < valuesCount; i++) {
            TZDBNameInfo *ninfo = const_cast<TZDBNameInfo*>((const TZDBNameInfo *)node->getValue(i));
            if (ninfo == nullptr) {
                continue;
            }
