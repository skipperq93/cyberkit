        ...
        for(i=0; i<UPRV_LENGTHOF(gb18030Ranges); range+=4, ++i) {
            if(range[0]<=(uint32_t)cp && (uint32_t)cp<=range[1]) {
                ...
                ucnv_fromUWriteBytes(cnv,
                                     bytes, 4, (char **)target, (const char *)targetLimit,
...
_EBCDICSwapLFNL(UConverterSharedData *sharedData, UErrorCode *pErrorCode) {
    ...
    if(mbcsTable->outputType==MBCS_OUTPUT_1) {
        ...
        if(!(
             ...
             EBCDIC_LF==MBCS_VALUE_2_FROM_STAGE_2(const_cast<uint8_t*>(bytes), stage2Entry, U_LF)
        )) {
            return false;
        }
        ...
        if(!(
             ...
             EBCDIC_NL==MBCS_VALUE_2_FROM_STAGE_2(const_cast<uint8_t*>(bytes), stage2Entry, U_NL)
        )) {
            return false;
        }
    }
...
writeStage3Roundtrip(const void *context, uint32_t value, UChar32 codePoints[32]) {
    UConverterMBCSTable *mbcsTable=const_cast<UConverterMBCSTable*>((const UConverterMBCSTable *)context);
    const uint16_t *table;
    uint32_t *stage2;
    uint8_t *bytes, *p;
    UChar32 c;
    ...
    table=mbcsTable->fromUnicodeTable;
    bytes=const_cast<uint8_t*>((const uint8_t *)mbcsTable->fromUnicodeBytes);
    ...
    for(i=0; i<=0x1f; ++value, ++i) {
        ...
        stage2=const_cast<uint32_t*>((const uint32_t *)table)+table[c>>10]+((c>>4)&0x3f);
        p=bytes;
...
ucnv_MBCSLoad(UConverterSharedData *sharedData,
    ...
    _MBCSHeader *header=const_cast<_MBCSHeader*>((const _MBCSHeader *)raw);
    uint32_t offset;
    ...
    if(mbcsTable->outputType==MBCS_OUTPUT_EXT_ONLY) {
        ...
        udata_getInfo(const_cast<UDataMemory*>((const UDataMemory *)sharedData->dataMemory), &info);
...
getTrail:
                ...
                /* MBCS_OUTPUT_2 */
                value=MBCS_VALUE_2_FROM_STAGE_2(const_cast<uint8_t*>(bytes), stage2Entry, c);
...
ucnv_MBCSSingleFromUnicodeWithOffsets(UConverterFromUnicodeArgs *pArgs,
    ...
    if((cnv->options&UCNV_OPTION_SWAP_LFNL)!=0) {
        ...
        results=(const uint16_t *)cnv->sharedData->mbcs.fromUnicodeBytes;
    }
...
getTrail:
    ...
    if(U_SUCCESS(*pErrorCode) && source<sourceLimit && target>=(const uint8_t *)pArgs->targetLimit) {
        /* target is full */
        *pErrorCode=U_BUFFER_OVERFLOW_ERROR;
    }
...
getTrail:
                ...
                case MBCS_OUTPUT_3:
                    ...
                    break;
                case MBCS_OUTPUT_4:
                    value=MBCS_VALUE_4_FROM_STAGE_2(const_cast<uint8_t*>(bytes), stage2Entry, c);
                    if(value<=0xff) {
                        length=1;
                    } else if(value<=0xffff) {
                        length=2;
                ...
                case MBCS_OUTPUT_3_EUC:
                    ...
                    break;
                case MBCS_OUTPUT_4_EUC:
                    p=MBCS_POINTER_3_FROM_STAGE_2(const_cast<uint8_t*>(bytes), stage2Entry, c);
                    value=((uint32_t)*p<<16)|((uint32_t)p[1]<<8)|p[2];
                    /* EUC 16-bit fixed-length representation applied to the first two bytes */
                    if(value<=0xff) {
                        length=1;
...
/* #if 0 because this is not currently used in ICU - reduce code, increase code coverage */
    ...
    if(c<=0xffff || (sharedData->mbcs.unicodeMask&UCNV_HAS_SUPPLEMENTARY)) {
        ...
        if(sharedData->mbcs.outputType==MBCS_OUTPUT_1) {
            value=MBCS_SINGLE_RESULT_FROM_U(table, (const uint16_t *)sharedData->mbcs.fromUnicodeBytes, c);
            /* is this code point assigned, or do we use fallbacks? */
            ...
            case MBCS_OUTPUT_2:
                value=MBCS_VALUE_2_FROM_STAGE_2(const_cast<uint8_t*>(sharedData->mbcs.fromUnicodeBytes), stage2Entry, c);
                if(value<=0xff) {
                    length=1;
                } else {
                    length=2;
...
ucnv_SBCSFromUTF8(UConverterFromUnicodeArgs *pFromUArgs,
    ...
    uint32_t asciiRoundtrips;
    uint16_t value = std::numeric_limits<uint16_t>::max(), minValue = 0;
    UBool hasSupplementary;
...
moreBytes:
                    while(toULength<toULimit) {
                         ...
                         * See ticket #7492.
                         */
                        if(source<(const uint8_t *)pToUArgs->sourceLimit) {
                            b=*source;
    ...
    if(U_SUCCESS(*pErrorCode) &&
            ...
            source<(sourceLimit=(const uint8_t *)pToUArgs->sourceLimit)) {
...
moreBytes:
                    ...
                    /* get the bytes and the length for the output */
                    /* MBCS_OUTPUT_2 */
                    value=MBCS_VALUE_2_FROM_STAGE_2(const_cast<uint16_t*>(results), stage2Entry, c);
