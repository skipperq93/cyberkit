                  ...
                  UErrorCode * err)
{
    ...
    if (reason <= UCNV_IRREGULAR)
    {
        ...
        else if (context == nullptr || (*((const char*)context) == UCNV_PRV_STOP_ON_ILLEGAL && reason == UCNV_UNASSIGNED))
        {
            *err = U_ZERO_ERROR;
        }
                 ...
                 UErrorCode * err)
{
    ...
    if (reason <= UCNV_IRREGULAR)
    {
        if (context == nullptr || (*((const char*)context) == UCNV_PRV_STOP_ON_ILLEGAL && reason == UCNV_UNASSIGNED))
        {
            *err = U_ZERO_ERROR;
        }
        /* else the caller must have set the error code accordingly. */
