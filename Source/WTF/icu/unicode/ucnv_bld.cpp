...
ucnv_deleteSharedConverterData(UConverterSharedData * deadSharedData)
{
    ...
    {
        UDataMemory *data = const_cast<UDataMemory*>((const UDataMemory*)deadSharedData->dataMemory);
        udata_close(data);
    }
...
ucnv_loadSharedData(const char *converterName,
...
#if U_CHARSET_IS_UTF8
        pArgs->name = "UTF-8";
        return const_cast<UConverterSharedData*>((const UConverterSharedData *)converterData[UCNV_UTF8]);
#else
    ...
    if (checkForAlgorithmic) {
        mySharedConverterData = const_cast<UConverterSharedData*>((const UConverterSharedData *)getAlgorithmicTypeFromName(pArgs->name));
    }
    if (mySharedConverterData == nullptr)
    {
...
ucnv_createAlgorithmicConverter(UConverter *myUConverter,
    ...
    cnv = ucnv_createConverterFromSharedData(
            myUConverter, const_cast<UConverterSharedData*>((const UConverterSharedData *)sharedData),
            &stackArgs, err);
