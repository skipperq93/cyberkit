...
struct CReg : public icu::UMemory {
    ...
    static UBool unreg(UCurrRegistryKey key) {
        ...
        while (*p) {
            if (*p == key) {
                *p = ((const CReg*)key)->next;
                delete (const CReg*)key;
                found = true;
                break;
            }
            p = &((*p)->next);
    ...
    *total_currency_symbol_count = 0;
    ...
    for (int32_t localeLevel = 0; ; ++localeLevel) {
        ...
        for (int32_t i=0; i<n; ++i) {
            ...
            iso = const_cast<char *>(ures_getKey(names));
            if (localeLevel == 0) {
            ...
            (*currencySymbols)[*total_currency_symbol_count].currencyName = const_cast<char16_t*>(s);
        ...
        for (int32_t i=0; i<ures_getSize(currencyMapArray); i++) {
            ...
            if (U_SUCCESS(localStatus)) {
                for (int32_t j=0; j<ures_getSize(currencyArray); j++) {
                    ...
                    localStatus = U_ZERO_ERROR;
                    uhash_put(isoCodes, const_cast<char16_t *>(isoCode), entry, &localStatus);
                }
            } else {
                *status = localStatus;
            }
