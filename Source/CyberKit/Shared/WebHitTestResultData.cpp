        ...
        , linkTextIndicator(linkTextIndicator)
{
    if (imageHandle && !imageHandle->isNull())
        imageSharedMemory = CyberKit::SharedMemory::map(WTFMove(*imageHandle), CyberKit::SharedMemory::Protection::ReadOnly);
}
...
std::optional<CyberKit::WebHitTestResultData> WebHitTestResultData::fromNavigationActionAndLocalFrame(const NavigationAction& navigationAction, CyberCore::LocalFrame* coreFrame)
{
    if (!coreFrame)
        return std::nullopt;
