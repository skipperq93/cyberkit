...
WebHitTestResultData::WebHitTestResultData(const CyberCore::HitTestResult& hitTestResult, const String& toolTipText)
...
WebHitTestResultData::WebHitTestResultData(const CyberCore::HitTestResult& hitTestResult, bool includeImage)
    : absoluteImageURL(hitTestResult.absoluteImageURL().string())
    , absolutePDFURL(hitTestResult.absolutePDFURL().string())
    , absoluteLinkURL(hitTestResult.absoluteLinkURL().string())
    , absoluteMediaURL(hitTestResult.absoluteMediaURL().string())
    ...
    , imageSize(0)
{
    ...
    if (Image* image = hitTestResult.image()) {
        ...
        if (buffer) {
            imageSharedMemory = CyberKit::SharedMemory::allocate(buffer->size());
...
void WebHitTestResultData::encode(IPC::Encoder& encoder) const
{
    ...
    CyberKit::SharedMemory::Handle imageHandle;
    if (imageSharedMemory && imageSharedMemory->data())
        imageSharedMemory->createHandle(imageHandle, CyberKit::SharedMemory::Protection::ReadOnly);

    encoder << CyberKit::SharedMemory::IPCHandle { WTFMove(imageHandle), imageSize };
...
bool WebHitTestResultData::decode(IPC::Decoder& decoder, WebHitTestResultData& hitTestResultData)
{
    ...
    CyberKit::SharedMemory::IPCHandle imageHandle;
    ...
    if (imageHandle.handle.isNull()) {
        ...
        hitTestResultData.imageSharedMemory = CyberKit::SharedMemory::map(imageHandle.handle, CyberKit::SharedMemory::Protection::ReadOnly);
    ...
    if (hasLinkTextIndicator) {
        Optional<CyberCore::TextIndicatorData> indicatorData;
        decoder >> indicatorData;
        if (!indicatorData)
            return false;

        hitTestResultData.linkTextIndicator = CyberCore::TextIndicator::create(*indicatorData);
    }
