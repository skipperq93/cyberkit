...
#include <CyberKitAdditions/UserMediaProcessManagerAdditions.cpp>
#endif
...
void UserMediaProcessManager::beginMonitoringCaptureDevices()
{
    ...
    std::call_once(onceFlag, [this] {
        m_captureDevices = CyberCore::RealtimeMediaSourceCenter::singleton().getMediaStreamDevices();

        CyberCore::RealtimeMediaSourceCenter::singleton().setDevicesChangedObserver([this]() {
            ...
            if (m_captureDevices.size() == oldDevices.size()) {
                ...
                for (auto &newDevice : m_captureDevices) {
                    if (newDevice.type() != CyberCore::CaptureDevice::DeviceType::Camera && newDevice.type() != CyberCore::CaptureDevice::DeviceType::Microphone)
                        continue;
