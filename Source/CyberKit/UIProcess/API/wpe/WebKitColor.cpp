...
{
    return CyberCore::convertToComponentBytes(CyberCore::SRGBA { static_cast<float>(color->red), static_cast<float>(color->green), static_cast<float>(color->blue), static_cast<float>(color->alpha) });
}
...
gboolean webkit_color_parse(CyberKitColor* color, const gchar* colorString)
{
    ...
    auto webCoreColor = CyberCore::CSSParser::parseColor({ colorString });
