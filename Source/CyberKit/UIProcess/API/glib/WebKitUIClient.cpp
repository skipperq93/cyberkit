...
private:
    ...
    void runJavaScriptAlert(WebPageProxy&, const String& message, WebFrameProxy*, CyberCore::SecurityOriginData&&, Function<void()>&& completionHandler) final
    {
        webkitWebViewRunJavaScriptAlert(m_webView, message.utf8(), WTFMove(completionHandler));
    }

    void runJavaScriptConfirm(WebPageProxy&, const String& message, WebFrameProxy*, CyberCore::SecurityOriginData&&, Function<void(bool)>&& completionHandler) final
    {
        webkitWebViewRunJavaScriptConfirm(m_webView, message.utf8(), WTFMove(completionHandler));
    }

    void runJavaScriptPrompt(WebPageProxy&, const String& message, const String& defaultValue, WebFrameProxy*, CyberCore::SecurityOriginData&&, Function<void(const String&)>&& completionHandler) final
    {
    ...
    void runBeforeUnloadConfirmPanel(WebPageProxy&, const String& message, WebFrameProxy*, CyberCore::SecurityOriginData&&, Function<void(bool)>&& completionHandler) final
    {
    ...
    bool runOpenPanel(WebPageProxy&, WebFrameProxy*, CyberCore::SecurityOriginData&&, API::OpenPanelParameters* parameters, WebOpenPanelResultListenerProxy* listener) final
    {
...
#if ENABLE(POINTER_LOCK)
    CyberKitPointerLockPermissionRequest* m_pointerLockPermissionRequest { nullptr };
#endif
};
