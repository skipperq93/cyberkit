...
/**
 * SECTION: CyberKitFaviconDatabase
 * @Short_description: A CyberKit favicon database
 * @Title: CyberKitFaviconDatabase
 *
 * #CyberKitFaviconDatabase provides access to the icons associated with
 * web sites.
 *
...
WEBKIT_DEFINE_TYPE(CyberKitFaviconDatabase, webkit_favicon_database, G_TYPE_OBJECT)

static void webkitFaviconDatabaseDispose(GObject* object)
{
    CyberKitFaviconDatabase* database = WEBKIT_FAVICON_DATABASE(object);
...
static RefPtr<cairo_surface_t> getIconSurfaceSynchronously(CyberKitFaviconDatabase* database, const String& pageURL, GError** error)
{
    ASSERT(RunLoop::isMain());

    // The exact size we pass is irrelevant to the iconDatabase code.
    // We must pass something greater than 0x0 to get an icon.
    auto iconData = database->priv->iconDatabase->synchronousIconForPageURL(pageURL, CyberCore::IntSize(1, 1));
...
static void deletePendingIconRequests(CyberKitFaviconDatabase* database, PendingIconRequestVector* requests, const String& pageURL)
{
    database->priv->pendingIconRequests.remove(pageURL);
    delete requests;
}

static void processPendingIconsForPageURL(CyberKitFaviconDatabase* database, const String& pageURL)
{
...
static void webkitFaviconDatabaseSetIconURLForPageURL(CyberKitFaviconDatabase* database, const String& iconURL, const String& pageURL)
{
...
class CyberKitIconDatabaseClient final : public IconDatabaseClient {
public:
    explicit CyberKitIconDatabaseClient(CyberKitFaviconDatabase* database)
        : m_database(database)
    {
    }
...
private:
    ...
    {
        CyberKitFaviconDatabasePrivate* priv = m_database->priv;
    ...
    CyberKitFaviconDatabase* m_database;
};
...
void webkitFaviconDatabaseOpen(CyberKitFaviconDatabase* database, const String& path)
{
    if (webkitFaviconDatabaseIsOpen(database))
        return;
    ...
    priv->iconDatabase->setClient(std::make_unique<CyberKitIconDatabaseClient>(database));
    ...
    if (!priv->iconDatabase->open(CyberCore::FileSystem::directoryName(path), CyberCore::FileSystem::pathGetFileName(path))) {
        priv->iconDatabase = nullptr;
        IconDatabase::allowDatabaseCleanup();
    }
}

bool webkitFaviconDatabaseIsOpen(CyberKitFaviconDatabase* database)
{
    return database->priv->iconDatabase && database->priv->iconDatabase->isOpen();
}

void webkitFaviconDatabaseSetPrivateBrowsingEnabled(CyberKitFaviconDatabase* database, bool enabled)
{
...
#if PLATFORM(GTK)
void webkitFaviconDatabaseGetLoadDecisionForIcon(CyberKitFaviconDatabase* database, const LinkIcon& icon, const String& pageURL, Function<void(bool)>&& completionHandler)
{
    if (!webkitFaviconDatabaseIsOpen(database)) {
...
void webkitFaviconDatabaseSetIconForPageURL(CyberKitFaviconDatabase* database, const LinkIcon& icon, API::Data& iconData, const String& pageURL)
{
    if (!webkitFaviconDatabaseIsOpen(database))
        return;
...
static PendingIconRequestVector* getOrCreatePendingIconRequests(CyberKitFaviconDatabase* database, const String& pageURL)
{
 ...
 * the #CyberKitWebContext associated with this #CyberKitFaviconDatabase
