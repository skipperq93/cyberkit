...
private:
    ...
    HashMap<uint64_t, GRefPtr<CyberKitURISchemeRequest>> m_requests;
};
...
static void webkitWebContextConstructed(GObject* object)
{
    ...
    if (!webkit_website_data_manager_is_ephemeral(priv->websiteDataManager.get()))
        CyberKit::LegacyGlobalSettings::singleton().setHSTSStorageDirectory(FileSystem::stringFromFileSystemRepresentation(webkit_website_data_manager_get_hsts_cache_directory(priv->websiteDataManager.get())));
...
CyberKitDownload* webkitWebContextStartDownload(CyberKitWebContext* context, const char* uri, WebPageProxy* initiatingPage)
{
    ...
    return webkitWebContextGetOrCreateDownload(&context->priv->processPool->download(CyberKit::WebsiteDataStore::defaultDataStore().get(), initiatingPage, request));
}
