...
#include "CyberKitInstallMissingMediaPluginsPermissionRequestPrivate.h"
...
/**
 * SECTION: CyberKitWebView
 * @Short_description: The central class of the WPE CyberKit and CyberKitGTK APIs
 * @Title: CyberKitWebView
 *
...
typedef HashMap<uint64_t, GRefPtr<CyberKitWebResource> > LoadingResourcesMap;
...
void CyberKitWebViewClient::handleDownloadRequest(WKWPE::View&, DownloadProxy& downloadProxy)
{
...
static void webkitWebViewRequestFavicon(CyberKitWebView* webView)
{
    ...
    CyberKitFaviconDatabase* database = webkit_web_context_get_favicon_database(priv->context.get());
...
static void webkitWebViewDisconnectFaviconDatabaseSignalHandlers(CyberKitWebView* webView)
{
    CyberKitWebViewPrivate* priv = webView->priv;
    if (priv->faviconChangedHandlerID)
...
static void notifyNotificationClosed(NotifyNotification*, CyberKitNotification* webNotification)
{
    g_object_set_data(G_OBJECT(webNotification), gNotifyNotificationID, nullptr);
    webkit_notification_close(webNotification);
}

static void notifyNotificationClicked(NotifyNotification*, char*, CyberKitNotification* webNotification)
{
    webkit_notification_clicked(webNotification);
}

static void webNotificationClosed(CyberKitNotification* webNotification)
{
...
static gboolean webkitWebViewShowNotification(CyberKitWebView*, CyberKitNotification* webNotification)
{
#if USE(LIBNOTIFY)
...
static void webkit_web_view_class_init(CyberKitWebViewClass* webViewClass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object(
            "related-view",
            _("Related WebView"),
            _("The related CyberKitWebView used when creating the view to share the same web process"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object(
            "settings",
            _("WebView settings"),
            _("The CyberKitSettings of the view"),
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object(
            ...
            _("The CyberKitUserContentManager of the view"),
     ...
     * #CyberKitWebView and it can't be changed. Note that all #CyberKitWebView<!-- -->s
     * created with an ephemeral #CyberKitWebContext will be ephemeral automatically.
...
void webkitWebViewLoadFailedWithTLSErrors(CyberKitWebView* webView, const char* failingURI, GError* error, GTlsCertificateFlags tlsErrors, GTlsCertificate* certificate)
{
    ...
    CyberKitTLSErrorsPolicy tlsErrorsPolicy = webkit_web_context_get_tls_errors_policy(webView->priv->context.get());
...
void webkitWebViewGetLoadDecisionForIcon(CyberKitWebView* webView, const LinkIcon& icon, Function<void(bool)>&& completionHandler)
{
    ...
    CyberKitFaviconDatabase* database = webkit_web_context_get_favicon_database(webView->priv->context.get());
...
Optional<CyberKitScriptDialogType> webkitWebViewGetCurrentScriptDialogType(CyberKitWebView* webView)
{
    if (!webView->priv->currentScriptDialog)
        return WTF::nullopt;
...
void webkitWebViewMouseTargetChanged(CyberKitWebView* webView, const WebHitTestResultData& hitTestResult, OptionSet<WebEvent::Modifier> modifiers)
{
#if PLATFORM(GTK)
...
void webkitWebViewHandleDownloadRequest(CyberKitWebView* webView, DownloadProxy* downloadProxy)
{
...
void webkitWebViewResourceLoadStarted(CyberKitWebView* webView, WebFrameProxy& frame, uint64_t resourceIdentifier, CyberKitURIRequest* request)
{
    CyberKitWebViewPrivate* priv = webView->priv;
    bool isMainResource = frame.isMainFrame() && !priv->mainResource;
    CyberKitWebResource* resource = webkitWebResourceCreate(frame, request, isMainResource);
    if (isMainResource)
...
CyberKitWebResource* webkitWebViewGetLoadingWebResource(CyberKitWebView* webView, uint64_t resourceIdentifier)
{
    GRefPtr<CyberKitWebResource> resource = webView->priv->loadingResourcesMap.get(resourceIdentifier);
    return resource.get();
}

void webkitWebViewRemoveLoadingWebResource(CyberKitWebView* webView, uint64_t resourceIdentifier)
{
    CyberKitWebViewPrivate* priv = webView->priv;
...
void webkitWebViewRequestInstallMissingMediaPlugins(CyberKitWebView* webView, InstallMissingMediaPluginsPermissionRequest& request)
{
#if ENABLE(VIDEO) && !USE(GSTREAMER_FULL)
    GRefPtr<CyberKitInstallMissingMediaPluginsPermissionRequest> installMediaPluginsPermissionRequest = adoptGRef(webkitInstallMissingMediaPluginsPermissionRequestCreate(request));
...
#if PLATFORM(GTK)
bool webkitWebViewShowOptionMenu(CyberKitWebView* webView, const IntRect& rect, CyberKitOptionMenu* menu, const GdkEvent* event)
{
...
static void webkitWebViewSynthesizeCompositionKeyPress(CyberKitWebView* webView, const String& text, Optional<Vector<CompositionUnderline>>&& underlines, Optional<EditingRange>&& selectionRange)
{
#if PLATFORM(GTK)
 ...
 * @web_view: a #CyberKitWebView
 *
 * Get whether a #CyberKitWebView is ephemeral. To create an ephemeral #CyberKitWebView you need to
 ...
 * property enabled. Only #CyberKitWebView<!-- -->s controlled by automation can be used in an
 * automation session.
 *
 ...
 * @web_view: a #CyberKitWebView
 *
 * Get the #CyberKitWebsiteDataManager associated to @web_view. If @web_view is not ephemeral,
...
void webkit_web_view_set_is_muted(CyberKitWebView* webView, gboolean muted)
{
    ...
    getPage(webView).setMuted(muted ? CyberCore::MediaProducer::AudioIsMuted : CyberCore::MediaProducer::NoneMuted);
 ...
 * @web_view: a #CyberKitWebView
 *
 * Obtains the #CyberKitBackForwardList associated with the given #CyberKitWebView. The
 * #CyberKitBackForwardList is owned by the #CyberKitWebView.
 *
 ...
 *
 * Sets the #CyberKitSettings to be applied to @web_view. The
...
void webkit_web_view_can_execute_editing_command(CyberKitWebView* webView, const char* command, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    getPage(webView).validateCommand(String::fromUTF8(command), [task = WTFMove(task)](const String&, bool isEnabled, int32_t, CyberKit::CallbackBase::Error) {
...
static void webkitWebViewRunJavaScriptWithParams(CyberKitWebView* webView, const gchar* script, RunJavaScriptParameters&& params, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    GRefPtr<GTask> task = adoptGRef(g_task_new(webView, cancellable, callback, userData));

    getPage(webView).runJavaScriptInMainFrame(WTFMove(params), [task = WTFMove(task)](API::SerializedScriptValue* serializedScriptValue, Optional<ExceptionDetails> details, CyberKit::CallbackBase::Error) {
...
void webkit_web_view_run_javascript_in_world(CyberKitWebView* webView, const gchar* script, const char* worldName, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    getPage(webView).runJavaScriptInFrameInScriptWorld({ String::fromUTF8(script), URL { }, false, WTF::nullopt, true }, WTF::nullopt, world.get(), [task = WTFMove(task)](API::SerializedScriptValue* serializedScriptValue, Optional<ExceptionDetails> details, CyberKit::CallbackBase::Error) {
        ExceptionDetails exceptionDetails;
        if (details)
            exceptionDetails = *details;
        webkitWebViewRunJavaScriptCallback(serializedScriptValue, exceptionDetails, task.get());
...
static void resourcesStreamReadCallback(GObject* object, GAsyncResult* result, gpointer userData)
{
    ...
    getPage(webView).runJavaScriptInMainFrame({ String::fromUTF8(reinterpret_cast<const gchar*>(outputStreamData)), URL { }, false, WTF::nullopt, true },
        [task](API::SerializedScriptValue* serializedScriptValue, Optional<ExceptionDetails> details, CyberKit::CallbackBase::Error) {
            ExceptionDetails exceptionDetails;
            if (details)
                exceptionDetails = *details;
            webkitWebViewRunJavaScriptCallback(serializedScriptValue, exceptionDetails, task.get());
...
void webkit_web_view_save(CyberKitWebView* webView, CyberKitSaveMode saveMode, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    getPage(webView).getContentsAsMHTMLData([task](API::Data* data, CyberKit::CallbackBase::Error) {
...
CyberKitDownload* webkit_web_view_download_uri(CyberKitWebView* webView, const char* uri)
{
    ...
    GRefPtr<CyberKitDownload> download = webkitWebContextStartDownload(webView->priv->context.get(), uri, &getPage(webView));
...
#if PLATFORM(GTK)
void webKitWebViewDidReceiveSnapshot(CyberKitWebView* webView, uint64_t callbackID, WebImage* webImage)
{
...
static inline unsigned webKitSnapshotOptionsToSnapshotOptions(CyberKitSnapshotOptions options)
{
...
static inline SnapshotRegion toSnapshotRegion(CyberKitSnapshotRegion region)
{
    switch (region) {
 ...
 * @user_data: (closure): the data to pass to callback function
 *
 * Send @message to the #CyberKitWebPage corresponding to @web_view. If @message is floating, it's consumed.
 *
