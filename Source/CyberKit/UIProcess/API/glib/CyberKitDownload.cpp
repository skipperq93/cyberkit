...
WEBKIT_DEFINE_TYPE(CyberKitDownload, webkit_download, G_TYPE_OBJECT)
...
CyberKitDownload* webkitDownloadCreate(DownloadProxy* downloadProxy)
{
    ASSERT(downloadProxy);
    CyberKitDownload* download = WEBKIT_DOWNLOAD(g_object_new(WEBKIT_TYPE_DOWNLOAD, NULL));
...
void webkitDownloadSetWebView(CyberKitDownload* download, CyberKitWebView* webView)
{
