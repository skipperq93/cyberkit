...
struct _CyberKitDownloadPrivate {
    ~_CyberKitDownloadPrivate()
    {
        if (decideDestinationCallback) {
            g_critical("Bug: application handled CyberKitDownload::decide-destination but failed to call webkit_download_set_destination() before the CyberKitDownload was destroyed");
...
static void maybeFinishDecideDestination(CyberKitDownload* download)
{
...
void webkitDownloadDecideDestinationWithSuggestedFilename(CyberKitDownload* download, CString&& suggestedFilename, CompletionHandler<void(AllowOverwrite, String)>&& completionHandler)
{
    if (download->priv->isCancelled)
        return;
 ...
 *
 * You can connect to #CyberKitDownload::created-destination to make
 ...
 */
void webkit_download_set_destination(CyberKitDownload* download, const gchar* destination)
{
