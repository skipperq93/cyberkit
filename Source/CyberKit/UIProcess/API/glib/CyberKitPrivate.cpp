...
#if PLATFORM(GTK)
unsigned toPlatformModifiers(OptionSet<CyberKit::WebEvent::Modifier> wkModifiers)
{
    unsigned modifiers = 0;
    if (wkModifiers.contains(CyberKit::WebEvent::Modifier::ShiftKey))
        modifiers |= GDK_SHIFT_MASK;
    if (wkModifiers.contains(CyberKit::WebEvent::Modifier::ControlKey))
        modifiers |= GDK_CONTROL_MASK;
    if (wkModifiers.contains(CyberKit::WebEvent::Modifier::AltKey))
        modifiers |= GDK_MOD1_MASK;
    if (wkModifiers.contains(CyberKit::WebEvent::Modifier::MetaKey))
        modifiers |= GDK_META_MASK;
    if (wkModifiers.contains(CyberKit::WebEvent::Modifier::CapsLockKey))
...
unsigned toCyberKitMouseButton(CyberKit::WebMouseEvent::Button button)
{
    switch (button) {
    case CyberKit::WebMouseEvent::Button::NoButton:
        return 0;
    case CyberKit::WebMouseEvent::Button::LeftButton:
        return 1;
    case CyberKit::WebMouseEvent::Button::MiddleButton:
        return 2;
    case CyberKit::WebMouseEvent::Button::RightButton:
        return 3;
    }
    ASSERT_NOT_REACHED();
    return 0;
