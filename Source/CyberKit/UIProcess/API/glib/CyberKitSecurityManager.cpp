...
/**
 * SECTION: CyberKitSecurityManager
 * @Short_description: Controls security settings in a #CyberKitWebContext
 * @Title: CyberKitSecurityManager
 *
...
WEBKIT_DEFINE_TYPE(CyberKitSecurityManager, webkit_security_manager, G_TYPE_OBJECT)
...
static void registerSecurityPolicyForURIScheme(CyberKitSecurityManager* manager, const char* scheme, SecurityPolicy policy)
{
    ...
    // We keep the CyberCore::SchemeRegistry of the UI process in sync with the
...
static bool checkSecurityPolicyForURIScheme(const char* scheme, SecurityPolicy policy)
{
    ...
    switch (policy) {
    case SecurityPolicyLocal:
        return CyberCore::SchemeRegistry::shouldTreatURLSchemeAsLocal(urlScheme);
    case SecurityPolicyNoAccess:
        return CyberCore::SchemeRegistry::shouldTreatURLSchemeAsNoAccess(urlScheme);
    case SecurityPolicyDisplayIsolated:
        return CyberCore::SchemeRegistry::shouldTreatURLSchemeAsDisplayIsolated(urlScheme);
    case SecurityPolicySecure:
        return CyberCore::SchemeRegistry::shouldTreatURLSchemeAsSecure(urlScheme);
    case SecurityPolicyCORSEnabled:
        return CyberCore::SchemeRegistry::shouldTreatURLSchemeAsCORSEnabled(urlScheme);
    case SecurityPolicyEmptyDocument:
        return CyberCore::SchemeRegistry::shouldLoadURLSchemeAsEmptyDocument(urlScheme);
    }
