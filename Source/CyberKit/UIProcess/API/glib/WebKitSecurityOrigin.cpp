...
/**
 * SECTION: CyberKitSecurityOrigin
 * @Short_description: A security boundary for websites
 * @Title: CyberKitSecurityOrigin
 *
...
struct _CyberKitSecurityOrigin {
    _CyberKitSecurityOrigin(Ref<CyberCore::SecurityOrigin>&& coreSecurityOrigin)
        : securityOrigin(WTFMove(coreSecurityOrigin))
    {
    }

    Ref<CyberCore::SecurityOrigin> securityOrigin;
    CString protocol;
    CString host;
...
CyberKitSecurityOrigin* webkitSecurityOriginCreate(Ref<CyberCore::SecurityOrigin>&& coreSecurityOrigin)
{
    CyberKitSecurityOrigin* origin = static_cast<CyberKitSecurityOrigin*>(fastMalloc(sizeof(CyberKitSecurityOrigin)));
    new (origin) CyberKitSecurityOrigin(WTFMove(coreSecurityOrigin));
    return origin;
}

CyberCore::SecurityOrigin& webkitSecurityOriginGetSecurityOrigin(CyberKitSecurityOrigin* origin)
{
    ASSERT(origin);
...
CyberKitSecurityOrigin* webkit_security_origin_new(const gchar* protocol, const gchar* host, guint16 port)
{
    ...
    return webkitSecurityOriginCreate(CyberCore::SecurityOrigin::create(String::fromUTF8(protocol), String::fromUTF8(host), optionalPort));
}
...
CyberKitSecurityOrigin* webkit_security_origin_new_for_uri(const gchar* uri)
{
    ...
    return webkitSecurityOriginCreate(CyberCore::SecurityOrigin::create(URL(URL(), String::fromUTF8(uri))));
}
 ...
 * #CyberKitSecurityOrigin constructed from either URI. It will also
