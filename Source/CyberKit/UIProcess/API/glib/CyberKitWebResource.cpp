...
WEBKIT_DEFINE_TYPE(CyberKitWebResource, webkit_web_resource, G_TYPE_OBJECT)
...
CyberKitWebResource* webkitWebResourceCreate(WebFrameProxy& frame, CyberKitURIRequest* request, bool isMainResource)
{
...
void webkitWebResourceSentRequest(CyberKitWebResource* resource, CyberKitURIRequest* request, CyberKitURIResponse* redirectResponse)
{
...
void webkitWebResourceSetResponse(CyberKitWebResource* resource, CyberKitURIResponse* response)
{
    resource->priv->response = response;
    g_object_notify(G_OBJECT(resource), "response");
}

void webkitWebResourceNotifyProgress(CyberKitWebResource* resource, guint64 bytesReceived)
{
...
void webkitWebResourceFailed(CyberKitWebResource* resource, GError* error)
{
    g_signal_emit(resource, signals[FAILED], 0, error);
    g_signal_emit(resource, signals[FINISHED], 0, NULL);
}

void webkitWebResourceFailedWithTLSErrors(CyberKitWebResource* resource, GTlsCertificateFlags tlsErrors, GTlsCertificate* certificate)
{
...
WebFrameProxy* webkitWebResourceGetFrame(CyberKitWebResource* resource)
{
