...
/**
 * SECTION: CyberKitPermissionRequest
 * @Short_description: A permission request
 * @Title: CyberKitPermissionRequest
...
static void webkit_permission_request_default_init(CyberKitPermissionRequestIface*)
{
}
...
void webkit_permission_request_allow(CyberKitPermissionRequest* request)
{
    ...
    CyberKitPermissionRequestIface* iface = WEBKIT_PERMISSION_REQUEST_GET_IFACE(request);
    if (iface->allow)
        iface->allow(request);
}
