...
/**
 * SECTION: CyberKitFileChooserRequest
 * @Short_description: A request to open a file chooser
 * @Title: CyberKitFileChooserRequest
 * @See_also: #CyberKitWebView
 *
 * Whenever the user interacts with an &lt;input type='file' /&gt;
 * HTML element, CyberKit will need to show a dialog to choose one or
...
WEBKIT_DEFINE_TYPE(CyberKitFileChooserRequest, webkit_file_chooser_request, G_TYPE_OBJECT)
...
void webkit_file_chooser_request_select_files(CyberKitFileChooserRequest* request, const gchar* const* files)
{
    ...
    for (int i = 0; files[i]; i++) {
        chosenFiles.append(CyberCore::decodeURLEscapeSequences(String::fromUTF8(files[i])));
 ...
 * @request: a #CyberKitFileChooserRequest
 *
 * Ask CyberKit to cancel the request. It's important to do this in case
