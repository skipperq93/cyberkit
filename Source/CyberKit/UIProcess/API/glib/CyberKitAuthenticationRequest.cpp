...
/**
 * SECTION: CyberKitAuthenticationRequest
 * @Short_description: Represents an authentication request
 * @Title: CyberKitAuthenticationRequest
...
struct _CyberKitAuthenticationRequestPrivate {
    ...
    CString host;
    CString realm;
    Optional<CyberCore::Credential> proposedCredential;
    Optional<CyberCore::Credential> acceptedCredential;
    Optional<bool> canSaveCredentials;
};

static guint signals[LAST_SIGNAL] = { 0, };

WEBKIT_DEFINE_TYPE(CyberKitAuthenticationRequest, webkit_authentication_request, G_TYPE_OBJECT)

static inline CyberKitAuthenticationScheme toCyberKitAuthenticationScheme(CyberCore::ProtectionSpaceAuthenticationScheme coreScheme)
{
    switch (coreScheme) {
    ...
    case CyberCore::ProtectionSpaceAuthenticationSchemeUnknown:
...
{
    auto* credential = webkitCredentialCreate(request->priv->acceptedCredential.valueOr(CyberCore::Credential()));
...
void webkit_authentication_request_authenticate(CyberKitAuthenticationRequest* request, CyberKitCredential* credential)
{
    ...
    request->priv->authenticationChallenge->listener().completeChallenge(CyberKit::AuthenticationChallengeDisposition::UseCredential, request->priv->acceptedCredential.valueOr(CyberCore::Credential()));
