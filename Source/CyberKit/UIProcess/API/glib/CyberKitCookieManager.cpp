...
/**
 * SECTION: CyberKitCookieManager
 * @Short_description: Defines how to handle cookies in a #CyberKitWebContext
 * @Title: CyberKitCookieManager
 *
...
WEBKIT_DEFINE_TYPE(CyberKitCookieManager, webkit_cookie_manager, G_TYPE_OBJECT)
...
static inline CyberKitCookieAcceptPolicy toCyberKitCookieAcceptPolicy(HTTPCookieAcceptPolicy httpPolicy)
{
    switch (httpPolicy) {
...
static inline HTTPCookieAcceptPolicy toHTTPCookieAcceptPolicy(CyberKitCookieAcceptPolicy kitPolicy)
{
    switch (kitPolicy) {
...
void webkit_cookie_manager_add_cookie(CyberKitCookieManager* manager, SoupCookie* cookie, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    const auto& processPools = webkitWebsiteDataManagerGetProcessPools(manager->priv->dataManager);
    processPools[0]->supplement<WebCookieManagerProxy>()->setCookies(manager->priv->sessionID(), { CyberCore::Cookie(cookie) }, [task = WTFMove(task)](CallbackBase::Error error) {
...
void webkit_cookie_manager_get_cookies(CyberKitCookieManager* manager, const gchar* uri, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    // of the process we access them from, so just use the first process pool.
    const auto& processPools = webkitWebsiteDataManagerGetProcessPools(manager->priv->dataManager);
    processPools[0]->supplement<WebCookieManagerProxy>()->getCookies(manager->priv->sessionID(), URL(URL(), String::fromUTF8(uri)), [task = WTFMove(task)](const Vector<CyberCore::Cookie>& cookies, CallbackBase::Error error) {
        if (error != CallbackBase::Error::None) {
            // This can only happen in cases where the web process is not available,
            // consider the operation "cancelled" from the point of view of the client.
            g_task_return_new_error(task.get(), G_IO_ERROR, G_IO_ERROR_CANCELLED, _("Operation was cancelled"));
...
void webkit_cookie_manager_delete_cookie(CyberKitCookieManager* manager, SoupCookie* cookie, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    // of the process we access them from, so just use the first process pool.
    const auto& processPools = webkitWebsiteDataManagerGetProcessPools(manager->priv->dataManager);
    processPools[0]->supplement<WebCookieManagerProxy>()->deleteCookie(manager->priv->sessionID(), CyberCore::Cookie(cookie), [task = WTFMove(task)](CallbackBase::Error error) {
        if (error != CallbackBase::Error::None) {
            // This can only happen in cases where the web process is not available,
            // consider the operation "cancelled" from the point of view of the client.
            g_task_return_new_error(task.get(), G_IO_ERROR, G_IO_ERROR_CANCELLED, _("Operation was cancelled"));
