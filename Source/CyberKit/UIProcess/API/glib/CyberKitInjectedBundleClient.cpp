...
private:
    static void didReceiveWebViewMessageFromInjectedBundle(CyberKitWebView* webView, const char* messageName, API::Dictionary& message)
    {
        if (g_str_equal(messageName, "DidInitiateLoadForResource")) {
            ...
            GRefPtr<CyberKitURIRequest> request = adoptGRef(webkitURIRequestCreateForResourceRequest(webRequest->resourceRequest()));
            ...
            API::UInt64* resourceIdentifier = static_cast<API::UInt64*>(message.get(String::fromUTF8("Identifier")));
            GRefPtr<CyberKitWebResource> resource = webkitWebViewGetLoadingWebResource(webView, resourceIdentifier->value());
            if (!resource)
                return;
            ...
            GRefPtr<CyberKitURIResponse> redirectResponse = webRedirectResponse ? adoptGRef(webkitURIResponseCreateForResourceResponse(webRedirectResponse->resourceResponse())) : 0;
            ...
            GRefPtr<CyberKitURIResponse> response = adoptGRef(webkitURIResponseCreateForResourceResponse(webResponse->resourceResponse()));
            ...
            GUniquePtr<GError> resourceError(g_error_new_literal(g_quark_from_string(platformError.domain().utf8().data()),
                toCyberKitError(platformError.errorCode()), platformError.localizedDescription().utf8().data()));
    ...
    void didReceiveMessageFromInjectedBundle(WebProcessPool&, const String& messageName, API::Object* messageBody) override
    {
        ...
        if (g_str_has_prefix(messageNameUTF8.data(), "WebPage.")) {
            ...
            CyberKitWebView* webView = webkitWebContextGetWebViewForPage(m_webContext, page);
            if (!webView)
                return;
...
{
    webkitWebContextGetProcessPool(webContext).setInjectedBundleClient(std::make_unique<CyberKitInjectedBundleClient>(webContext));
}
