...
class WebViewClient final : public API::ViewClient {
    WTF_MAKE_FAST_ALLOCATED;
public:
    explicit WebViewClient(CyberKitWebView* webView)
        : m_webView(webView)
    {
    }
...
static void webkitWebViewCancelAuthenticationRequest(CyberKitWebView* webView)
{
...
WebPageProxy* webkitWebViewCreateNewPage(CyberKitWebView* webView, const WindowFeatures& windowFeatures, CyberKitNavigationAction* navigationAction)
{
...
#if PLATFORM(GTK)
static void contextMenuDismissed(GtkMenuShell*, CyberKitWebView* webView)
{
...
void webkit_web_view_run_javascript(CyberKitWebView* webView, const gchar* script, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    GRefPtr<GTask> task = adoptGRef(g_task_new(webView, cancellable, callback, userData));
    getPage(webView).runJavaScriptInMainFrame({ String::fromUTF8(script), false, WTF::nullopt, true }, [task = WTFMove(task)](API::SerializedScriptValue* serializedScriptValue, Optional<ExceptionDetails> details, CyberKit::CallbackBase::Error) {
...
void webkit_web_view_run_javascript_in_world(CyberKitWebView* webView, const gchar* script, const char* worldName, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    GRefPtr<GTask> task = adoptGRef(g_task_new(webView, cancellable, callback, userData));
    getPage(webView).runJavaScriptInMainFrameScriptWorld({ String::fromUTF8(script), false, WTF::nullopt, true }, String::fromUTF8(worldName), [task = WTFMove(task)](API::SerializedScriptValue* serializedScriptValue, Optional<ExceptionDetails> details, CyberKit::CallbackBase::Error) {
        ExceptionDetails exceptionDetails;
        if (details)
            exceptionDetails = *details;
        webkitWebViewRunJavaScriptCallback(serializedScriptValue, exceptionDetails, task.get());
