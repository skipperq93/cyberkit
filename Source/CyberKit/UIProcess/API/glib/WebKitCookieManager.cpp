 ...
 * Since: 2.42
 */
void webkit_cookie_manager_replace_cookies(CyberKitCookieManager* manager, GList* cookies, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    g_return_if_fail(WEBKIT_IS_COOKIE_MANAGER(manager));
    g_return_if_fail(cookies);

    Vector<CyberCore::Cookie> webCookies;
    for (GList* it = cookies; it; it = g_list_next(it))
        webCookies.append(CyberCore::Cookie(reinterpret_cast<SoupCookie*>(it->data)));
 ...
 * Since: 2.42
 */
gboolean webkit_cookie_manager_replace_cookies_finish(CyberKitCookieManager* manager, GAsyncResult* result, GError** error)
{
    g_return_val_if_fail(WEBKIT_IS_COOKIE_MANAGER(manager), FALSE);
    g_return_val_if_fail(g_task_is_valid(result, manager), FALSE);
 ...
 * Since: 2.42
 */
void webkit_cookie_manager_get_all_cookies(CyberKitCookieManager* manager, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    g_return_if_fail(WEBKIT_IS_COOKIE_MANAGER(manager));

    GRefPtr<GTask> task = adoptGRef(g_task_new(manager, cancellable, callback, userData));
    manager->priv->cookieStore().getAllCookies([task = WTFMove(task)](const Vector<CyberCore::Cookie>& cookies) {
        GList* cookiesList = nullptr;
        for (auto& cookie : cookies)
            cookiesList = g_list_prepend(cookiesList, cookie.toSoupCookie());
 ...
 * Since: 2.42
 */
GList* webkit_cookie_manager_get_all_cookies_finish(CyberKitCookieManager* manager, GAsyncResult* result, GError** error)
{
    g_return_val_if_fail(WEBKIT_IS_COOKIE_MANAGER(manager), nullptr);
    g_return_val_if_fail(g_task_is_valid(result, manager), nullptr);
