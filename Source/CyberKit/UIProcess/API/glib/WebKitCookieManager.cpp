...
void webkit_cookie_manager_add_cookie(CyberKitCookieManager* manager, SoupCookie* cookie, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    const auto& processPools = webkitWebsiteDataManagerGetProcessPools(manager->priv->dataManager);
    processPools[0]->supplement<WebCookieManagerProxy>()->setCookies(manager->priv->sessionID(), { CyberCore::Cookie(cookie) }, [task = WTFMove(task)]() {
...
void webkit_cookie_manager_get_cookies(CyberKitCookieManager* manager, const gchar* uri, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    // of the process we access them from, so just use the first process pool.
    const auto& processPools = webkitWebsiteDataManagerGetProcessPools(manager->priv->dataManager);
    processPools[0]->supplement<WebCookieManagerProxy>()->getCookies(manager->priv->sessionID(), URL(URL(), String::fromUTF8(uri)), [task = WTFMove(task)](const Vector<CyberCore::Cookie>& cookies) {
...
void webkit_cookie_manager_delete_cookie(CyberKitCookieManager* manager, SoupCookie* cookie, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    // of the process we access them from, so just use the first process pool.
    const auto& processPools = webkitWebsiteDataManagerGetProcessPools(manager->priv->dataManager);
    processPools[0]->supplement<WebCookieManagerProxy>()->deleteCookie(manager->priv->sessionID(), CyberCore::Cookie(cookie), [task = WTFMove(task)]() {
        g_task_return_boolean(task.get(), TRUE);
    });
}
