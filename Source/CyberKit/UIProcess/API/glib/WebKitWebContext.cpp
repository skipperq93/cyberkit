...
/**
 * SECTION: CyberKitWebContext
 * @Short_description: Manages aspects common to all #CyberKitWebView<!-- -->s
 * @Title: CyberKitWebContext
 *
 * The #CyberKitWebContext manages all aspects common to all
 * #CyberKitWebView<!-- -->s.
 *
 * You can define the #CyberKitCacheModel and #CyberKitProcessModel with
...
class CyberKitURISchemeHandler: public RefCounted<CyberKitURISchemeHandler> {
public:
    CyberKitURISchemeHandler(CyberKitURISchemeRequestCallback callback, void* userData, GDestroyNotify destroyNotify)
        : m_callback(callback)
        , m_userData(userData)
    ...
    void performCallback(CyberKitURISchemeRequest* request)
    {
        ASSERT(m_callback);
...
typedef HashMap<uint64_t, GRefPtr<CyberKitURISchemeRequest> > URISchemeRequestMap;
...
struct _CyberKitWebContextPrivate {
    ...
    std::unique_ptr<CyberKitGeolocationProvider> geolocationProvider;
#endif
    ...
    CyberKitProcessModel processModel;
    unsigned processCountLimit;

    HashMap<uint64_t, CyberKitWebView*> webViews;
...
WEBKIT_DEFINE_TYPE(CyberKitWebContext, webkit_web_context, G_TYPE_OBJECT)
...
static void webkitWebContextConstructed(GObject* object)
{
    ...
    configuration.setInjectedBundlePath(CyberCore::FileSystem::stringFromFileSystemRepresentation(bundleFilename.get()));
    ...
    if (priv->websiteDataManager && !webkit_website_data_manager_is_ephemeral(priv->websiteDataManager.get())) {
        configuration.setLocalStorageDirectory(CyberCore::FileSystem::stringFromFileSystemRepresentation(webkit_website_data_manager_get_local_storage_directory(priv->websiteDataManager.get())));
        configuration.setDiskCacheDirectory(CyberCore::FileSystem::pathByAppendingComponent(CyberCore::FileSystem::stringFromFileSystemRepresentation(webkit_website_data_manager_get_disk_cache_directory(priv->websiteDataManager.get())), networkCacheSubdirectory));
        configuration.setApplicationCacheDirectory(CyberCore::FileSystem::stringFromFileSystemRepresentation(webkit_website_data_manager_get_offline_application_cache_directory(priv->websiteDataManager.get())));
        configuration.setIndexedDBDatabaseDirectory(CyberCore::FileSystem::stringFromFileSystemRepresentation(webkit_website_data_manager_get_indexeddb_directory(priv->websiteDataManager.get())));
        configuration.setWebSQLDatabaseDirectory(CyberCore::FileSystem::stringFromFileSystemRepresentation(webkit_website_data_manager_get_websql_directory(priv->websiteDataManager.get())));
    } else if (!priv->localStorageDirectory.isNull())
        configuration.setLocalStorageDirectory(CyberCore::FileSystem::stringFromFileSystemRepresentation(priv->localStorageDirectory.data()));
...
#if ENABLE(GEOLOCATION)
    priv->geolocationProvider = std::make_unique<CyberKitGeolocationProvider>(priv->processPool->supplement<WebGeolocationManagerProxy>());
#endif
    priv->notificationProvider = std::make_unique<CyberKitNotificationProvider>(priv->processPool->supplement<WebNotificationManagerProxy>(), webContext);
...
static void webkit_web_context_class_init(CyberKitWebContextClass* webContextClass)
{
    ...
    g_object_class_install_property(
        ...
        g_param_spec_object(
            ...
            _("The CyberKitWebsiteDataManager associated with this context"),
 ...
 *
 * Create a new ephemeral #CyberKitWebContext. An ephemeral #CyberKitWebContext is a context
...
void webkit_web_context_set_automation_allowed(CyberKitWebContext* context, gboolean allowed)
{
    ...
    if (allowed) {
        ...
        context->priv->automationClient = std::make_unique<CyberKitAutomationClient>(context);
    } else
 ...
 */
void webkit_web_context_set_cache_model(CyberKitWebContext* context, CyberKitCacheModel model)
{
...
void webkit_web_context_set_network_proxy_settings(CyberKitWebContext* context, CyberKitNetworkProxyMode proxyMode, CyberKitNetworkProxySettings* proxySettings)
{
    ...
    case WEBKIT_NETWORK_PROXY_MODE_NO_PROXY:
        priv->processPool->setNetworkProxySettings(CyberCore::SoupNetworkProxySettings(CyberCore::SoupNetworkProxySettings::Mode::NoProxy));
        break;
...
typedef HashMap<DownloadProxy*, GRefPtr<CyberKitDownload> > DownloadsMap;
 ...
 * will not be associated to any #CyberKitWebView, if you are interested in
 * starting a download from a particular #CyberKitWebView use
...
CyberKitDownload* webkit_web_context_download_uri(CyberKitWebContext* context, const gchar* uri)
{
    ...
    GRefPtr<CyberKitDownload> download = webkitWebContextStartDownload(context, uri, nullptr);
...
static void webkitWebContextEnableIconDatabasePrivateBrowsingIfNeeded(CyberKitWebContext* context, CyberKitWebView* webView)
{
...
static void webkitWebContextDisableIconDatabasePrivateBrowsingIfNeeded(CyberKitWebContext* context, CyberKitWebView* webView)
{
    if (webkit_web_context_is_ephemeral(context))
        return;
    if (!webkit_web_view_is_ephemeral(webView))
...
void webkit_web_context_set_favicon_database_directory(CyberKitWebContext* context, const gchar* path)
{
    ...
    String directoryPath = CyberCore::FileSystem::stringFromFileSystemRepresentation(path);
    ...
    if (directoryPath.isEmpty()) {
        ...
        directoryPath = CyberCore::FileSystem::stringFromFileSystemRepresentation(databaseDirectory.get());
    }
    ...
    webkitFaviconDatabaseOpen(priv->faviconDatabase.get(), CyberCore::FileSystem::stringFromFileSystemRepresentation(faviconDatabasePath.get()));
 ...
 */
void webkit_web_context_set_additional_plugins_directory(CyberKitWebContext* context, const char* directory)
{
    g_return_if_fail(WEBKIT_IS_WEB_CONTEXT(context));
    g_return_if_fail(directory);

#if ENABLE(NETSCAPE_PLUGIN_API)
    context->priv->processPool->setAdditionalPluginsDirectory(CyberCore::FileSystem::stringFromFileSystemRepresentation(directory));
#endif
}
...
void webkit_web_context_register_uri_scheme(CyberKitWebContext* context, const char* scheme, CyberKitURISchemeRequestCallback callback, gpointer userData, GDestroyNotify destroyNotify)
{
    ...
    RefPtr<CyberKitURISchemeHandler> handler = adoptRef(new CyberKitURISchemeHandler(callback, userData, destroyNotify));
 ...
 */
void webkit_web_context_set_disk_cache_directory(CyberKitWebContext* context, const char* directory)
{
    g_return_if_fail(WEBKIT_IS_WEB_CONTEXT(context));
    g_return_if_fail(directory);

    context->priv->processPool->configuration().setDiskCacheDirectory(CyberCore::FileSystem::pathByAppendingComponent(CyberCore::FileSystem::stringFromFileSystemRepresentation(directory), networkCacheSubdirectory));
}
#endif
...
void webkit_web_context_allow_tls_certificate_for_host(CyberKitWebContext* context, GTlsCertificate* certificate, const gchar* host)
{
    ...
    auto webCertificateInfo = WebCertificateInfo::create(CyberCore::CertificateInfo(certificate, static_cast<GTlsCertificateFlags>(0)));
 ...
 *
 * Specifies a process model for WebViews, which CyberKit will use to
 ...
 *
 * Returns: the current #CyberKitProcessModel
 *
 * Since: 2.4
 */
...
CyberKitDownload* webkitWebContextGetOrCreateDownload(DownloadProxy* downloadProxy)
{
    GRefPtr<CyberKitDownload> download = downloadsMap().get(downloadProxy);
    if (download)
        return download.get();
...
CyberKitDownload* webkitWebContextStartDownload(CyberKitWebContext* context, const char* uri, WebPageProxy* initiatingPage)
{
...
void webkitWebContextStartLoadingCustomProtocol(CyberKitWebContext* context, uint64_t customProtocolID, const CyberCore::ResourceRequest& resourceRequest, LegacyCustomProtocolManagerProxy& manager)
{
    GRefPtr<CyberKitURISchemeRequest> request = adoptGRef(webkitURISchemeRequestCreate(customProtocolID, context, resourceRequest, manager));
    String scheme(String::fromUTF8(webkit_uri_scheme_request_get_scheme(request.get())));
    RefPtr<CyberKitURISchemeHandler> handler = context->priv->uriSchemeHandlers.get(scheme);
    ASSERT(handler.get());
...
void webkitWebContextStopLoadingCustomProtocol(CyberKitWebContext* context, uint64_t customProtocolID)
{
    GRefPtr<CyberKitURISchemeRequest> request = context->priv->uriSchemeRequests.get(customProtocolID);
    if (!request.get())
        return;
    webkitURISchemeRequestCancel(request.get());
}

void webkitWebContextInvalidateCustomProtocolRequests(CyberKitWebContext* context, LegacyCustomProtocolManagerProxy& manager)
{
...
void webkitWebContextDidFinishLoadingCustomProtocol(CyberKitWebContext* context, uint64_t customProtocolID)
{
    context->priv->uriSchemeRequests.remove(customProtocolID);
}

bool webkitWebContextIsLoadingCustomProtocol(CyberKitWebContext* context, uint64_t customProtocolID)
{
    return context->priv->uriSchemeRequests.get(customProtocolID);
}

void webkitWebContextCreatePageForWebView(CyberKitWebContext* context, CyberKitWebView* webView, CyberKitUserContentManager* userContentManager, CyberKitWebView* relatedView)
{
