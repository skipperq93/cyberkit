...
CyberKitWebsiteDataManager* webkitWebsiteDataManagerCreate(Ref<WebsiteDataStoreConfiguration>&& configuration)
{
    CyberKitWebsiteDataManager* manager = WEBKIT_WEBSITE_DATA_MANAGER(g_object_new(WEBKIT_TYPE_WEBSITE_DATA_MANAGER, nullptr));
    manager->priv->websiteDataStore = API::WebsiteDataStore::createLegacy(WTFMove(configuration));

    return manager;
}

API::WebsiteDataStore& webkitWebsiteDataManagerGetDataStore(CyberKitWebsiteDataManager* manager)
{
    ...
    if (!priv->websiteDataStore) {
        ...
        configuration->setLocalStorageDirectory(!priv->localStorageDirectory ?
            API::WebsiteDataStore::defaultLocalStorageDirectory() : CyberCore::FileSystem::stringFromFileSystemRepresentation(priv->localStorageDirectory.get()));
        configuration->setNetworkCacheDirectory(!priv->diskCacheDirectory ?
            API::WebsiteDataStore::defaultNetworkCacheDirectory() : CyberCore::FileSystem::pathByAppendingComponent(CyberCore::FileSystem::stringFromFileSystemRepresentation(priv->diskCacheDirectory.get()), networkCacheSubdirectory));
        configuration->setApplicationCacheDirectory(!priv->applicationCacheDirectory ?
            API::WebsiteDataStore::defaultApplicationCacheDirectory() : CyberCore::FileSystem::stringFromFileSystemRepresentation(priv->applicationCacheDirectory.get()));
        configuration->setWebSQLDatabaseDirectory(!priv->webSQLDirectory ?
            API::WebsiteDataStore::defaultWebSQLDatabaseDirectory() : CyberCore::FileSystem::stringFromFileSystemRepresentation(priv->webSQLDirectory.get()));
...
const gchar* webkit_website_data_manager_get_disk_cache_directory(CyberKitWebsiteDataManager* manager)
{
    ...
    if (!priv->diskCacheDirectory) {
        ...
        priv->diskCacheDirectory.reset(g_strdup(CyberCore::FileSystem::directoryName(API::WebsiteDataStore::defaultNetworkCacheDirectory()).utf8().data()));
    }
