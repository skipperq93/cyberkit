...
CyberKit::WebsiteDataStore& webkitWebsiteDataManagerGetDataStore(CyberKitWebsiteDataManager* manager)
{
    ...
    if (!priv->websiteDataStore) {
        ...
        configuration->setLocalStorageDirectory(!priv->localStorageDirectory ?
            CyberKit::WebsiteDataStore::defaultLocalStorageDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->localStorageDirectory.get()));
        configuration->setNetworkCacheDirectory(!priv->diskCacheDirectory ?
            CyberKit::WebsiteDataStore::defaultNetworkCacheDirectory() : FileSystem::pathByAppendingComponent(FileSystem::stringFromFileSystemRepresentation(priv->diskCacheDirectory.get()), networkCacheSubdirectory));
        configuration->setApplicationCacheDirectory(!priv->applicationCacheDirectory ?
            CyberKit::WebsiteDataStore::defaultApplicationCacheDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->applicationCacheDirectory.get()));
        configuration->setIndexedDBDatabaseDirectory(!priv->indexedDBDirectory ?
            CyberKit::WebsiteDataStore::defaultIndexedDBDatabaseDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->indexedDBDirectory.get()));
        configuration->setWebSQLDatabaseDirectory(!priv->webSQLDirectory ?
            CyberKit::WebsiteDataStore::defaultWebSQLDatabaseDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->webSQLDirectory.get()));
        configuration->setHSTSStorageDirectory(!priv->hstsCacheDirectory ?
            CyberKit::WebsiteDataStore::defaultHSTSDirectory() : FileSystem::stringFromFileSystemRepresentation(priv->hstsCacheDirectory.get()));
        configuration->setMediaKeysStorageDirectory(CyberKit::WebsiteDataStore::defaultMediaKeysStorageDirectory());
