...
#include <CyberCore/CaptureDeviceWithCapabilities.h>
...
void UserMediaPermissionRequestManagerProxy::checkUserMediaPermissionForSpeechRecognition(CyberCore::FrameIdentifier frameIdentifier, const CyberCore::SecurityOrigin& requestingOrigin, const CyberCore::SecurityOrigin& topOrigin, const CyberCore::CaptureDevice& device, CompletionHandler<void(bool)>&& completionHandler)
{
    ...
    auto request = UserMediaPermissionRequestProxy::create(*this, ObjectIdentifier<CyberCore::UserMediaRequestIdentifierType>(0), frameIdentifier, frameIdentifier, requestingOrigin.isolatedCopy(), topOrigin.isolatedCopy(), Vector<CyberCore::CaptureDevice> { device }, { }, { }, WTFMove(completionHandler));
...
void UserMediaPermissionRequestManagerProxy::clearUserMediaPermissionRequestHistory(CyberCore::PermissionName permissionName)
{
    m_deniedRequests.removeAllMatching([permissionName](auto& request) {
        return (request.isAudioDenied && permissionName == CyberCore::PermissionName::Microphone) || (request.isVideoDenied && permissionName == CyberCore::PermissionName::Camera);
    });
    m_grantedRequests.removeAllMatching([permissionName](auto& request) {
        return (request->requiresAudioCapture() && permissionName == CyberCore::PermissionName::Microphone) || (request->requiresVideoCapture() && permissionName == CyberCore::PermissionName::Camera);
    });
}
    ...
    platformGetMediaStreamDevices(revealIdsAndLabels, [logIdentifier = LOGIDENTIFIER, this, weakThis = WeakPtr { *this }, revealIdsAndLabels, completion = WTFMove(completion)](auto&& devicesWithCapabilities) mutable {
        ...
        for (auto& deviceWithCapabilities : devicesWithCapabilities) {
            ...
            if (!revealIdsAndLabels) {
                ...
                if (device.type() == CyberCore::CaptureDevice::DeviceType::Speaker && !haveMicrophoneDevice(devicesWithCapabilities, device.groupId()))
                    continue;
            }
