...
#if ENABLE(MEDIA_STREAM)
    m_page.process().send(Messages::WebPage::UserMediaAccessWasDenied(request.userMediaID(), toCyberCore(reason), invalidConstraint), m_page.webPageID());
#else
    UNUSED_PARAM(reason);
...
Vector<CaptureDevice> UserMediaPermissionRequestManagerProxy::computeFilteredDeviceList(bool revealIdsAndLabels, const String& deviceIDHashSalt)
{
    ...
    for (const auto& device : devices) {
        if (!device.enabled() || (device.type() != CyberCore::CaptureDevice::DeviceType::Camera && device.type() != CyberCore::CaptureDevice::DeviceType::Microphone))
            continue;
