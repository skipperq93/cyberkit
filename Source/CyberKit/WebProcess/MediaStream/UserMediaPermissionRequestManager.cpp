...
static constexpr OptionSet<CyberCore::ActivityState::Flag> focusedActiveWindow = { CyberCore::ActivityState::IsFocused, CyberCore::ActivityState::WindowIsActive };
...
void UserMediaPermissionRequestManager::userMediaAccessWasDenied(uint64_t requestID, CyberCore::UserMediaRequest::MediaAccessDenialReason reason, String&& invalidConstraint)
{
    auto request = m_idToUserMediaRequestMap.take(requestID);
    if (!request)
        return;
...
void UserMediaPermissionRequestManager::cancelMediaDevicesEnumeration(CyberCore::MediaDevicesEnumerationRequest& request)
{
...
{
    auto identifier = generateObjectIdentifier<CyberCore::UserMediaClient::DeviceChangeObserverTokenType>();
