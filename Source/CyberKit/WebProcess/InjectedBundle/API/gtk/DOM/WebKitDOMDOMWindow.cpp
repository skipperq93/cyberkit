...
#include <CyberCore/JSDOMGlobalObject.h>
...
typedef struct _CyberKitDOMDOMWindowPrivate {
    RefPtr<CyberCore::LocalDOMWindow> coreObject;
...
CyberKitDOMDOMWindow* kit(CyberCore::LocalDOMWindow* obj)
{
    if (!obj)
        return 0;
...
{
    if (!windowProxy || !is<CyberCore::LocalDOMWindow>(windowProxy->window()))
        return nullptr;

    return kit(downcast<CyberCore::LocalDOMWindow>(windowProxy->window()));
}
...
CyberCore::LocalDOMWindow* core(CyberKitDOMDOMWindow* request)
{
    return request ? static_cast<CyberCore::LocalDOMWindow*>(WEBKIT_DOM_OBJECT(request)->coreObject) : 0;
}

CyberKitDOMDOMWindow* wrapDOMWindow(CyberCore::LocalDOMWindow* coreObject)
{
    ASSERT(coreObject);
...
static gboolean webkit_dom_dom_window_dispatch_event(CyberKitDOMEventTarget* target, CyberKitDOMEvent* event, GError** error)
{
    ...
    if (!coreEvent)
        return false;
    CyberCore::LocalDOMWindow* coreTarget = static_cast<CyberCore::LocalDOMWindow*>(WEBKIT_DOM_OBJECT(target)->coreObject);
...
static GObject* webkit_dom_dom_window_constructor(GType type, guint constructPropertiesCount, GObjectConstructParam* constructProperties)
{
    ...
    CyberKitDOMDOMWindowPrivate* priv = WEBKIT_DOM_DOM_WINDOW_GET_PRIVATE(object);
    priv->coreObject = static_cast<CyberCore::LocalDOMWindow*>(WEBKIT_DOM_OBJECT(object)->coreObject);
...
CyberKitDOMDOMWindow* webkit_dom_dom_window_get_opener(CyberKitDOMDOMWindow* self)
{
    ...
    RefPtr gobjectResult = downcast<CyberCore::LocalDOMWindow>(openerWindowProxy->window());
    return CyberKit::kit(gobjectResult.get());
}
...
CyberKitDOMDOMWindow* webkit_dom_dom_window_get_parent(CyberKitDOMDOMWindow* self)
{
    ...
    RefPtr gobjectResult = downcast<CyberCore::LocalDOMWindow>(parentWindowProxy->window());
    return CyberKit::kit(gobjectResult.get());
}
...
CyberKitDOMDOMWindow* webkit_dom_dom_window_get_top(CyberKitDOMDOMWindow* self)
{
    ...
    RefPtr gobjectResult = downcast<CyberCore::LocalDOMWindow>(topWindowProxy->window());
    return CyberKit::kit(gobjectResult.get());
}
...
gboolean webkit_dom_dom_window_webkit_message_handlers_post_message(CyberKitDOMDOMWindow* window, const gchar* handlerName, const gchar* message)
{
    ...
    CyberCore::LocalDOMWindow* domWindow = CyberKit::core(window);
