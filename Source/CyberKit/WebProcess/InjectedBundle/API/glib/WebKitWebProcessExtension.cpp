...
#include "config.h"
#include "CyberKitWebProcessExtension.h"
...
/**
 * CyberKitWebProcessExtension:
 *
 * Represents an extension of the web process.
 *
 * CyberKitWebProcessExtension is a loadable module for the web process. It allows you to execute code in the
 * web process and being able to use the DOM API, to change any request or to inject custom
 * JavaScript code, for example.
 *
 * To create a CyberKitWebProcessExtension you should write a module with an initialization function that could
 * be either webkit_web_process_extension_initialize() with prototype #CyberKitWebProcessExtensionInitializeFunction or
 * webkit_web_process_extension_initialize_with_user_data() with prototype #CyberKitWebProcessExtensionInitializeWithUserDataFunction.
 ...
 * ```c
 * static void
 * web_page_created_callback (CyberKitWebProcessExtension *extension,
 ...
 * webkit_web_process_extension_initialize (CyberKitWebProcessExtension *extension)
 * {
 ...
 *
 * CyberKit has to know where it can find the created CyberKitWebProcessExtension. To do so you
...
struct _CyberKitWebProcessExtensionPrivate {
    WebPageMap pages;
...
WEBKIT_DEFINE_FINAL_TYPE(CyberKitWebProcessExtension, webkit_web_process_extension, G_TYPE_OBJECT, GObject)

static void webkit_web_process_extension_class_init(CyberKitWebProcessExtensionClass* klass)
{
    /**
     * CyberKitWebProcessExtension::page-created:
     * @extension: the #CyberKitWebProcessExtension on which the signal is emitted
    ...
    /**
     * CyberKitWebProcessExtension::user-message-received:
     * @extension: the #CyberKitWebProcessExtension on which the signal is emitted
...
public:
    explicit WebProcessExtensionInjectedBundleClient(CyberKitWebProcessExtension* extension)
...
private:
    ...
    CyberKitWebProcessExtension* m_extension;
};

CyberKitWebProcessExtension* webkitWebProcessExtensionCreate(InjectedBundle* bundle)
{
    CyberKitWebProcessExtension* extension = WEBKIT_WEB_PROCESS_EXTENSION(g_object_new(WEBKIT_TYPE_WEB_PROCESS_EXTENSION, NULL));
    bundle->setClient(makeUnique<WebProcessExtensionInjectedBundleClient>(extension));
    return extension;
}

void webkitWebProcessExtensionDidReceiveUserMessage(CyberKitWebProcessExtension* extension, UserMessage&& message)
{
...
void webkitWebProcessExtensionSetGarbageCollectOnPageDestroy(CyberKitWebProcessExtension* extension)
{
#if ENABLE(DEVELOPER_MODE)
 ...
 * @extension: a #CyberKitWebProcessExtension
 ...
 * Since: 2.40
 */
CyberKitWebPage* webkit_web_process_extension_get_page(CyberKitWebProcessExtension* extension, guint64 pageID)
{
    g_return_val_if_fail(WEBKIT_IS_WEB_PROCESS_EXTENSION(extension), 0);

    CyberKitWebProcessExtensionPrivate* priv = extension->priv;
 ...
 * Since: 2.40
 */
void webkit_web_process_extension_send_message_to_context(CyberKitWebProcessExtension* extension, CyberKitUserMessage* message, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
 ...
 * Since: 2.40
 */
CyberKitUserMessage* webkit_web_process_extension_send_message_to_context_finish(CyberKitWebProcessExtension* extension, GAsyncResult* result, GError** error)
{
