...
{
    Vector<String> modulePaths = CyberCore::FileSystem::listDirectory(webExtensionsDirectory, String("*.so"));
    for (size_t i = 0; i < modulePaths.size(); ++i) {
        if (CyberCore::FileSystem::fileExists(modulePaths[i]))
...
static void parseUserData(API::Object* userData, String& webExtensionsDirectory, GRefPtr<GVariant>& initializationUserData)
{
    ...
    webExtensionsDirectory = CyberCore::FileSystem::stringFromFileSystemRepresentation(directory);
