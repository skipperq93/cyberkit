...
#include "config.h"
#include "CyberKitSoupRequestInputStream.h"
...
struct _CyberKitSoupRequestInputStreamPrivate {
...
G_DEFINE_TYPE(CyberKitSoupRequestInputStream, webkit_soup_request_input_stream, G_TYPE_MEMORY_INPUT_STREAM)

static void webkitSoupRequestInputStreamReadAsyncResultComplete(GTask* task, void* buffer, gsize count)
{
    CyberKitSoupRequestInputStream* stream = WEBKIT_SOUP_REQUEST_INPUT_STREAM(g_task_get_source_object(task));
...
static void webkitSoupRequestInputStreamPendingReadAsyncComplete(CyberKitSoupRequestInputStream* stream)
{
...
static bool webkitSoupRequestInputStreamHasDataToRead(CyberKitSoupRequestInputStream* stream)
{
    return stream->priv->bytesRead < stream->priv->bytesReceived;
}

static bool webkitSoupRequestInputStreamIsWaitingForData(CyberKitSoupRequestInputStream* stream)
{
...
static void webkitSoupRequestInputStreamReadAsync(GInputStream* inputStream, void* buffer, gsize count, int /*priority*/, GCancellable* cancellable, GAsyncReadyCallback callback, gpointer userData)
{
    ...
    CyberKitSoupRequestInputStream* stream = WEBKIT_SOUP_REQUEST_INPUT_STREAM(inputStream);
...
{
    WEBKIT_SOUP_REQUEST_INPUT_STREAM(object)->priv->~CyberKitSoupRequestInputStreamPrivate();
    G_OBJECT_CLASS(webkit_soup_request_input_stream_parent_class)->finalize(object);
}

static void webkit_soup_request_input_stream_init(CyberKitSoupRequestInputStream* stream)
{
    CyberKitSoupRequestInputStreamPrivate* priv = G_TYPE_INSTANCE_GET_PRIVATE(stream, WEBKIT_TYPE_SOUP_REQUEST_INPUT_STREAM, CyberKitSoupRequestInputStreamPrivate);
    stream->priv = priv;
    new (priv) CyberKitSoupRequestInputStreamPrivate();
}

static void webkit_soup_request_input_stream_class_init(CyberKitSoupRequestInputStreamClass* requestStreamClass)
{
    ...
    g_type_class_add_private(requestStreamClass, sizeof(CyberKitSoupRequestInputStreamPrivate));
}

GInputStream* webkitSoupRequestInputStreamNew(uint64_t contentLength)
{
    CyberKitSoupRequestInputStream* stream = WEBKIT_SOUP_REQUEST_INPUT_STREAM(g_object_new(WEBKIT_TYPE_SOUP_REQUEST_INPUT_STREAM, NULL));
    stream->priv->contentLength = contentLength;
    return G_INPUT_STREAM(stream);
}

void webkitSoupRequestInputStreamAddData(CyberKitSoupRequestInputStream* stream, const void* data, size_t dataLength)
{
    ASSERT(RunLoop::isMain());
...
void webkitSoupRequestInputStreamDidFailWithError(CyberKitSoupRequestInputStream* stream, const CyberCore::ResourceError& resourceError)
{
...
bool webkitSoupRequestInputStreamFinished(CyberKitSoupRequestInputStream* stream)
{
