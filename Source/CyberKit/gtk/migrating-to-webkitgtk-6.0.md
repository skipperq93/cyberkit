Title: Migrating CyberKitGTK Applications to GTK 4
Slug: migrating-to-webkitgtk-6.0

# Migrating CyberKitGTK Applications to GTK 4

This document contains guidance to application developers looking to migrate
applications that use CyberKitGTK from GTK 3 to GTK 4.

webkitgtk-6.0 is a new API version of CyberKitGTK designed for use with GTK 4 and
...
[method@CyberKit.WebView.evaluate_javascript] or
[method@CyberKit.WebView.call_async_javascript_function] in the UI process, or
...
[method@CyberKit.WebContext.add_path_to_sandbox] to mount them in the sandbox.

## Mandatory Process Swap on Cross-site Navigation

The `CyberKitWebContext:process-swap-on-cross-site-navigation-enabled` property
...
`CyberKitWebContext:process-swap-on-cross-site-navigation-enabled` property
...
[signal@CyberKit.WebView::context-menu] and [signal@CyberKit.WebView::show-option-menu]
no longer have a [type@Gdk.Event] parameter. Adjust your signal handlers
accordingly.

## Changes to CyberKitWebView Construction
...
may directly use `g_object_new()` instead. [ctor@CyberKit.WebView.new] remains.

## Network Session API

CyberKit now uses a single global network process for all web contexts, and different
network sessions can be created and used in the same network process. All the networking
APIs have been moved from [type@CyberKit.WebContext] and [type@CyberKit.WebsiteDataManager] to the new class
[type@CyberKit.NetworkSession]. There's a default global persistent session that you can get with
[func@CyberKit.NetworkSession.get_default]. You can also create new sessions with
[ctor@CyberKit.NetworkSession.new] for persistent sessions and [ctor@CyberKit.NetworkSession.new_ephemeral]
for ephemeral sessions. It's no longer possible to create a [type@CyberKit.WebsiteDataManager]; it's now
created by the [type@CyberKit.NetworkSession] automatically at construction time. The [type@CyberKit.NetworkSession]
to be used must be passed to the [type@CyberKit.WebView] as a construct parameter. You can pass the
same [type@CyberKit.NetworkSession] object to several web views to use the same session. The only exception
is automation mode, which uses its own ephemeral session that is configured by the automation
session capabilities. If you notice that your application uses [type@CyberKit.WebContext] or
[type@CyberKit.WebsiteDataManager] APIs that no longer exist, look for replacement APIs
in [type@CyberKit.NetworkSession].

`webkit_web_context_clear_cache()` does not have a direct replacement, but you
can use [method@CyberKit.WebsiteDataManager.clear] to achieve the same effect.

The `CyberKitWebContext::download-started` signal has been removed. Use
[signal@CyberKit.NetworkSession::download-started] instead.

## Favicon Database

[type@CyberKit.FaviconDatabase] is now owned by [type@CyberKit.WebsiteDataManager]
instead of [type@CyberKit.WebContext]. Use
[method@CyberKit.WebsiteDataManager.set_favicons_enabled] to enable the favicon
database.
...
[enum@CyberKit.HardwareAccelerationPolicy]. You may still use
[method@CyberKit.Settings.set_hardware_acceleration_policy] to enable or disable
...
removed. CyberKit will draw scrollbars that match the Adwaita GTK theme.
...
been removed. Instead, [method@CyberKit.UserContentManager.register_script_message_handler]
and [method@CyberKit.UserContentManager.unregister_script_message_handler] have
gained parameters to specify the script world to use.

## Download Destination

[method@CyberKit.Download.set_destination], [method@CyberKit.Download.get_destination],
[property@CyberKit.Download:destination], and [signal@CyberKit.Download::created-destination]
now all use a filesystem path rather than a URI. All uses must be updated accordingly.

## JavaScript Results

CyberKitJavascriptResult has been removed. [signal@CyberKit.UserContentManager::script-message-received]
now directly returns a [class@JSC.Value] instead.

## Web Process Extension

CyberKitWebExtension has been renamed to CyberKitWebProcessExtension to avoid
confusion with WebExtensions, which are unrelated. This affects many methods and
signals of other objects as well, e.g. [signal@CyberKit.WebContext::initialize-web-process-extensions].
...
Beware that as of CyberKitGTK 2.40, the entire web process API may unfortunately
