...
#include <CyberCore/CyberKitSoupRequestGeneric.h>
#include <wtf/NeverDestroyed.h>

namespace CyberKit {
using namespace CyberCore;

LegacyCustomProtocolManager::WebSoupRequestAsyncData::WebSoupRequestAsyncData(GRefPtr<GTask>&& requestTask, CyberKitSoupRequestGeneric* requestGeneric)
...
class CustomProtocolRequestClient final : public CyberKitSoupRequestGenericClient {
public:
...
private:
    ...
    {
        CyberKitSoupRequestGeneric* request = WEBKIT_SOUP_REQUEST_GENERIC(g_task_get_source_object(task.get()));
...
{
    static_cast<CyberKitSoupRequestGenericClass*>(g_type_class_ref(WEBKIT_TYPE_SOUP_REQUEST_GENERIC))->client = &CustomProtocolRequestClient::singleton();
...
void LegacyCustomProtocolManager::registerScheme(const String& scheme)
{
    ...
    NetworkStorageSession::forEach([](const CyberCore::NetworkStorageSession& session) {
...
void LegacyCustomProtocolManager::didReceiveResponse(uint64_t customProtocolID, const ResourceResponse& response, uint32_t)
{
    ...
    CyberKitSoupRequestGeneric* request = WEBKIT_SOUP_REQUEST_GENERIC(g_task_get_source_object(data->task.get()));
