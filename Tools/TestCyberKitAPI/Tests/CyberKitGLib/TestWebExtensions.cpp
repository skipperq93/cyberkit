...
static void testDocumentLoadedSignal(WebViewTest* test, gconstpointer)
{
    ...
    test->loadHtml("<html><head><title>CyberKitGTK Web Extensions Test</title></head><body></body></html>", nullptr);
...
static void testWebExtensionWindowObjectCleared(WebViewTest* test, gconstpointer)
{
    ...
    CyberKitJavascriptResult* javascriptResult = test->runJavaScriptAndWaitUntilFinished("window.echo('Foo');", &error.outPtr());
...
static void testWebExtensionIsolatedWorld(WebViewTest* test, gconstpointer)
{
    ...
    CyberKitJavascriptResult* javascriptResult = test->runJavaScriptAndWaitUntilFinished("document.getElementById('console').innerHTML", 0);
    g_assert_nonnull(javascriptResult);
    GUniquePtr<char> valueString(WebViewTest::javascriptResultToCString(javascriptResult));
    g_assert_cmpstr(valueString.get(), ==, "Foo");
...
#if PLATFORM(GTK)
static gboolean permissionRequestCallback(CyberKitWebView*, CyberKitPermissionRequest* request, WebViewTest* test)
{
    if (!WEBKIT_IS_INSTALL_MISSING_MEDIA_PLUGINS_PERMISSION_REQUEST(request))
        return FALSE;

    test->assertObjectIsDeletedWhenTestFinishes(G_OBJECT(request));
    CyberKitInstallMissingMediaPluginsPermissionRequest* missingPluginsRequest = WEBKIT_INSTALL_MISSING_MEDIA_PLUGINS_PERMISSION_REQUEST(request);
...
static void testInstallMissingPluginsPermissionRequest(WebViewTest* test, gconstpointer)
{
    ...
    GUniquePtr<char> baseURI(g_strconcat("file://", Test::getResourcesDir(Test::CyberKit2Resources).data(), "/", nullptr));
...
public:
    ...
    GRefPtr<CyberKitUserMessage> m_receivedViewMessage;
...
void beforeAll()
{
    WebViewTest::add("CyberKitWebExtension", "dom-document-title", testWebExtensionGetTitle);
#if PLATFORM(GTK)
    WebViewTest::add("CyberKitWebExtension", "dom-input-element-is-user-edited", testWebExtensionInputElementIsUserEdited);
#endif
    WebViewTest::add("CyberKitWebExtension", "document-loaded-signal", testDocumentLoadedSignal);
    WebViewTest::add("CyberKitWebView", "web-process-crashed", testCyberKitWebViewProcessCrashed);
    WebViewTest::add("CyberKitWebExtension", "window-object-cleared", testWebExtensionWindowObjectCleared);
    WebViewTest::add("CyberKitWebExtension", "isolated-world", testWebExtensionIsolatedWorld);
#if PLATFORM(GTK)
    WebViewTest::add("CyberKitWebView", "install-missing-plugins-permission-request", testInstallMissingPluginsPermissionRequest);
#endif
    WebViewTest::add("CyberKitWebExtension", "form-controls-associated-signal", testWebExtensionFormControlsAssociated);
    FormSubmissionTest::add("CyberKitWebExtension", "form-submission-steps", testWebExtensionFormSubmissionSteps);
    WebViewTest::add("CyberKitWebExtension", "page-id", testWebExtensionPageID);
    UserMessageTest::add("CyberKitWebExtension", "user-messages", testWebExtensionUserMessages);
}
