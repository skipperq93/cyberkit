...
static void testWebContextGetPlugins(PluginsTest* test, gconstpointer)
{
    ...
    GRefPtr<CyberKitPlugin> testPlugin;
    for (GList* item = plugins; item; item = g_list_next(item)) {
        CyberKitPlugin* plugin = WEBKIT_PLUGIN(item->data);
        test->assertObjectIsDeletedWhenTestFinishes(G_OBJECT(plugin));
        if (!g_strcmp0(webkit_plugin_get_name(plugin), "CyberKit Test PlugIn")) {
            testPlugin = plugin;
            break;
        }
    }
    ...
    g_assert_cmpstr(webkit_plugin_get_description(testPlugin.get()), ==, "Simple Netscape® plug-in that handles test content for CyberKit");
    ...
    CyberKitMimeInfo* mimeInfo = static_cast<CyberKitMimeInfo*>(mimeInfoList->data);
    ...
    mimeInfo = static_cast<CyberKitMimeInfo*>(mimeInfoList->data);
...
static void testWebContextLanguages(WebViewTest* test, gconstpointer)
{
    ...
    GUniqueOutPtr<GError> error;
    CyberKitJavascriptResult* javascriptResult = test->runJavaScriptAndWaitUntilFinished("Intl.DateTimeFormat().resolvedOptions().locale", &error.outPtr());
    g_assert_nonnull(javascriptResult);
    g_assert_no_error(error.get());
...
static void consoleMessageReceivedCallback(CyberKitUserContentManager*, CyberKitJavascriptResult* message, Vector<CyberKitJavascriptResult*>* result)
{
...
static void testWebContextSecurityFileXHR(WebViewTest* test, gconstpointer)
{
    ...
    Vector<CyberKitJavascriptResult*> consoleMessages;
    ...
    GUniqueOutPtr<GError> error;
    CyberKitJavascriptResult* javascriptResult = test->runJavaScriptAndWaitUntilFinished(xhr.get(), &error.outPtr());
    g_assert_nonnull(javascriptResult);
    g_assert_no_error(error.get());
...
static void testWebContextProxySettings(ProxyTest* test, gconstpointer)
{
    ...
    CyberKitNetworkProxySettings* settings = webkit_network_proxy_settings_new(proxyURI.get(), nullptr);
...
#if ENABLE(NETSCAPE_PLUGIN_API)
    PluginsTest::add("CyberKitWebContext", "get-plugins", testWebContextGetPlugins);
#endif
