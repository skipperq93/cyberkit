...
public:
    static std::unique_ptr<WebProcessTest> create() { return std::make_unique<CyberKitDOMDOMWindowTest>(); }
...
private:
    ...
    bool testSignals(CyberKitWebExtension* extension, GVariant* args)
    {
        notify("ready", "");
    ...
    bool testDispatchEvent(CyberKitWebExtension* extension, GVariant* args)
    {
        notify("ready", "");
    ...
    bool testGetComputedStyle(CyberKitWebExtension* extension, GVariant* args)
    {
        CyberKitWebPage* page = webkit_web_extension_get_page(extension, webPageFromArgs(args));
        g_assert_true(WEBKIT_IS_WEB_PAGE(page));
        CyberKitDOMDocument* document = webkit_web_page_get_dom_document(page);
    ...
    virtual bool runTest(const char* testName, CyberKitWebExtension* extension, GVariant* args)
    {
