...
class CyberKitDOMClientRectTest : public WebProcessTest {
public:
    static std::unique_ptr<WebProcessTest> create() { return std::unique_ptr<CyberKitDOMClientRectTest>(new CyberKitDOMClientRectTest()); }

private:
    static void checkClientRectPosition(CyberKitDOMClientRect* clientRect)
    {
    ...
    bool testDivBoundingClientRectPosition(CyberKitWebPage* page)
    {
        ...
        CyberKitDOMElement* div = webkit_dom_document_get_element_by_id(document, "rect");
        g_assert(WEBKIT_DOM_IS_HTML_ELEMENT(div));
        assertObjectIsDeletedWhenTestFinishes(G_OBJECT(div));

        GRefPtr<CyberKitDOMClientRect> clientRect = adoptGRef(webkit_dom_element_get_bounding_client_rect(div));
    ...
    bool testDivClientRectsPositionAndLength(CyberKitWebPage* page)
    {
        CyberKitDOMDocument* document = webkit_web_page_get_dom_document(page);
        g_assert(WEBKIT_DOM_IS_DOCUMENT(document));
        assertObjectIsDeletedWhenTestFinishes(G_OBJECT(document));
        ...
        GRefPtr<CyberKitDOMClientRectList> clientRectList = adoptGRef(webkit_dom_element_get_client_rects(div));
        ...
        GRefPtr<CyberKitDOMClientRect> clientRect = adoptGRef(webkit_dom_client_rect_list_item(clientRectList.get(), 0));
        g_assert(WEBKIT_DOM_IS_CLIENT_RECT(clientRect.get()));
        assertObjectIsDeletedWhenTestFinishes(G_OBJECT(clientRect.get()));
        checkClientRectPosition(clientRect.get());

        // Getting the clientRect twice should return the same pointer.
        GRefPtr<CyberKitDOMClientRect> clientRect2 = adoptGRef(webkit_dom_client_rect_list_item(clientRectList.get(), 0));
...
{
    REGISTER_TEST(CyberKitDOMClientRectTest, "CyberKitDOMClientRect/div-bounding-client-rect-position");
    REGISTER_TEST(CyberKitDOMClientRectTest, "CyberKitDOMClientRect/div-client-rects-position-and-length");
}
