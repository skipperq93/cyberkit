...
public:
    ...
    static void resourceReceivedDataCallback(CyberKitWebResource* resource, guint64 bytesReceived, ResourcesTest* test)
    {
    ...
    virtual void resourceReceivedData(CyberKitWebResource* resource, guint64 bytesReceived)
    {
    }
...
public:
    ...
    void resourceReceivedData(CyberKitWebResource* resource, guint64 bytesReceived)
    {
        if (resource != m_resource)
            return;
    ...
    void resourceFinished(CyberKitWebResource* resource)
    {
        ...
        if (!m_loadEvents.contains(Failed)) {
            CyberKitURIResponse* response = webkit_web_resource_get_response(m_resource.get());
