...
static gboolean contextMenuCallback(CyberKitWebPage* page, CyberKitContextMenu* menu, CyberKitWebHitTestResult* hitTestResult, gpointer)
{
    ...
    if (!g_strcmp0(pageURI, "ContextMenuTestNode")) {
        CyberKitDOMNode* node = webkit_web_hit_test_result_get_node(hitTestResult);
...
static void formControlsAssociatedForFrameCallback(CyberKitWebPage* webPage, GPtrArray* formElements, CyberKitFrame* frame, CyberKitWebExtension* extension)
{
...
static void handleFormSubmissionCallback(CyberKitWebPage* webPage, DelayedSignalType delayedSignalType, const char* methodName, const char* formID, CyberKitFrame* sourceFrame, CyberKitFrame* targetFrame, GPtrArray* textFieldNames, GPtrArray* textFieldValues, CyberKitWebExtension* extension)
{
...
static void willSubmitFormCallback(CyberKitWebPage* webPage, CyberKitDOMElement* formElement, CyberKitFormSubmissionStep step, CyberKitFrame* sourceFrame, CyberKitFrame* targetFrame, GPtrArray* textFieldNames, GPtrArray* textFieldValues, CyberKitWebExtension* extension)
{
#if PLATFORM(GTK)
    G_GNUC_BEGIN_IGNORE_DEPRECATIONS;
...
static void windowObjectCleared(CyberKitScriptWorld* world, CyberKitWebPage* page, CyberKitFrame* frame, gpointer)
{
...
static void methodCallCallback(GDBusConnection* connection, const char* sender, const char* objectPath, const char* interfaceName, const char* methodName, GVariant* parameters, GDBusMethodInvocation* invocation, gpointer userData)
{
    ...
    if (!g_strcmp0(methodName, "GetTitle")) {
        ...
        CyberKitDOMNode* node = webkit_dom_node_for_js_value(jsInputElement.get());
...
extern "C" void webkit_web_extension_initialize_with_user_data(CyberKitWebExtension* extension, GVariant* userData)
{
