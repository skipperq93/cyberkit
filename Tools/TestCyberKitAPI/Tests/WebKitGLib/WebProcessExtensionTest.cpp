...
typedef CyberKitWebExtension CyberKitWebProcessExtension;
#endif
...
static void documentLoadedCallback(CyberKitWebPage* webPage, CyberKitWebProcessExtension* extension)
{
#if PLATFORM(GTK) && !USE(GTK4)
...
static void uriChangedCallback(CyberKitWebPage* webPage, GParamSpec* pspec, CyberKitWebProcessExtension* extension)
{
...
#if !ENABLE(2022_GLIB_API)
static void formControlsAssociatedForFrameCallback(CyberKitWebPage*, GPtrArray*, CyberKitFrame*, CyberKitWebProcessExtension*)
{
    g_assert_not_reached();
}
#endif

static void formControlsAssociatedCallback(CyberKitWebFormManager*, CyberKitFrame*, GPtrArray* formElements, CyberKitWebProcessExtension* extension)
{
...
static void handleFormSubmissionCallback(CyberKitWebProcessExtension* extension, DelayedSignalType delayedSignalType, const char* methodName, JSCValue* form, CyberKitFrame* sourceFrame, CyberKitFrame* targetFrame)
{
...
static void willSendSubmitEventCallback(CyberKitWebFormManager*, JSCValue* form, CyberKitFrame* sourceFrame, CyberKitFrame* targetFrame, CyberKitWebProcessExtension* extension)
{
...
static void willSubmitFormCallback(CyberKitWebFormManager*, JSCValue* form, CyberKitFrame* sourceFrame, CyberKitFrame* targetFrame, CyberKitWebProcessExtension* extension)
{
...
static gboolean pageMessageReceivedCallback(CyberKitWebPage* webPage, CyberKitUserMessage* message, CyberKitWebProcessExtension* extension)
{
...
static void pageCreatedCallback(CyberKitWebProcessExtension* extension, CyberKitWebPage* webPage, gpointer)
{
...
static gboolean extensionMessageReceivedCallback(CyberKitWebProcessExtension* extension, CyberKitUserMessage* message)
{
    const char* messageName = webkit_user_message_get_name(message);
...
#endif
            [](GObject* object, GAsyncResult* result, gpointer) {
                auto* extension = reinterpret_cast<CyberKitWebProcessExtension*>(object);
                GUniqueOutPtr<GError> error;
#if ENABLE(2022_GLIB_API)
                GRefPtr<CyberKitUserMessage> reply = adoptGRef(webkit_web_process_extension_send_message_to_context_finish(extension, result, &error.outPtr()));
#else
...
static void windowObjectCleared(CyberKitScriptWorld* world, CyberKitWebPage* page, CyberKitFrame* frame, CyberKitWebProcessExtension* extension)
{
...
static CyberKitWebPage* getWebPage(CyberKitWebProcessExtension* extension, uint64_t pageID, GDBusMethodInvocation* invocation)
{
#if ENABLE(2022_GLIB_API)
    CyberKitWebPage* page = webkit_web_process_extension_get_page(extension, pageID);
#else
...
static void methodCallCallback(GDBusConnection* connection, const char* sender, const char* objectPath, const char* interfaceName, const char* methodName, GVariant* parameters, GDBusMethodInvocation* invocation, gpointer userData)
{
    ...
    if (!g_strcmp0(methodName, "GetTitle")) {
        ...
        CyberKitWebPage* page = getWebPage(static_cast<CyberKitWebProcessExtension*>(userData), pageID, invocation);
        if (!page)
            return;
...
#if ENABLE(2022_GLIB_API)
extern "C" WTF_EXPORT_DECLARATION void webkit_web_process_extension_initialize_with_user_data(CyberKitWebProcessExtension* extension, GVariant* userData)
#else
extern "C" WTF_EXPORT_DECLARATION void webkit_web_extension_initialize_with_user_data(CyberKitWebProcessExtension* extension, GVariant* userData)
#endif
{
    CyberKitScriptWorld* isolatedWorld = webkit_script_world_new_with_name("WebProcessExtensionTestScriptWorld");
