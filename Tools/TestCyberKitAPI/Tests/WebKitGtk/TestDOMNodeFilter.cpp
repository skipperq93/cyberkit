...
{
    g_assert(test->runWebProcessTest("CyberKitDOMNodeFilter", "tree-walker", testHTML));
}

static void testCyberKitDOMNodeFilterNodeIterator(WebViewTest* test, gconstpointer)
{
    g_assert(test->runWebProcessTest("CyberKitDOMNodeFilter", "node-iterator", testHTML));
}
