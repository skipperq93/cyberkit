/*
 This file is licensed under the BSD 3-Clause license. Please view the original at:
 https://github.com/ProcursusTeam/uikittools-ng/blob/91abeceb451638a3fd992c4a3406a2208ec511a1/uicache.m
 */
...
NSDictionary *dumpEntitlementsFromBinaryAtPath(NSString *binaryPath) {
	...
	#ifdef CYBERKIT
	//TODO: Actually define the path to file
	NSString *file = [[[binaryPath stringByDeletingLastPathComponent] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"script_fakesigner.entitlements"];
	NSDictionary *entitlements = [NSDictionary dictionaryWithContentsOfFile:file];
	#else
...
void registerPath(NSString *path, BOOL unregister, BOOL forceSystem) {
	...
	if (!appBundleID && verbose) {
		printf("Could not obtain bundle ID from %s\n", [path stringByAppendingPathComponent:@"Info.plist"].UTF8String);
	}
